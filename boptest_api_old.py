...
class RestApi(object):
    ...
    def put_forecast(self, N: int, dt: int):
    ...
    def put_results(self, ts, tf, points):
        return requests.put('{0}/results'.format(self.url),
                    json={
                        'point_names': points,
                        'start_time': ts,
                        'final_time': tf
    ...
    @property
    def y_names(self):
        return [v + "_y" for k, v in self.y.items()]
    
    @property
    def u_names(self):
        return [v + "_u" for k, v in self.u.items()]
...
class Boptest(RestApi):
    ...
    def get_results(self, tf, ts=0, resample=True):
        ''' Get results. '''
