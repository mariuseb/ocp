...
for alpha in alphas:
    ...
    obj_v = cp.Maximize(cp.log_det(S) - cp.trace(S*V))
    obj_w = cp.Maximize(cp.log_det(P) - cp.trace(P@W))

    # Set constraint.
    constraints_v = [cp.sum(cp.abs(S)) <= alpha]
    constraints_w = [cp.sum(cp.abs(P)) <= alpha]

    # Form and solve optimization problem
    prob_v = cp.Problem(obj_v, constraints_v)
    prob_w = cp.Problem(obj_w, constraints_w)
    #prob.solve(solver=cp.CVXOPT)
    prob_v.solve(solver=cp.SCS, verbose=True)
    if prob_v.status != cp.OPTIMAL:
        raise Exception('CVXPY Error')
    
    prob_w.solve(solver=cp.SCS, verbose=True)
    if prob_w.status != cp.OPTIMAL:
        raise Exception('CVXPY Error')
