...
# scale x:
# spread in x:
a_x = data["ub_Ti"].max() - data["lb_Ti"].min()
b_x = data["lb_Ti"].min()
...
for k in range(N):
    ...
    for j in range(1,d+1):
       ...
       #fj, qj = f(Xc[j-1]*x_nom, P*p_nom, Uk*u_nom)
       fj, qj = f(Xc[j-1]*a_x + b_x, P*p_nom, Uk*u_nom)
       #fj, qj = f(Xc[j-1]*a_x, P*p_nom, Uk*u_nom)
       #g.append(h*fj - xp*x_nom)
       g.append(h*fj - xp*a_x)
       #lbg.append([0, 0])
       #ubg.append([0, 0])
       lbg.append([0])
       ubg.append([0])
...
opts = dict(qpsol='qrqp', qpsol_options=dict(print_iter=False,error_on_fail=False), print_time=True)
...
#ax.plot(tgrid, x_opt[0]*x_nom, '--')
ax.plot(tgrid, x_opt[0]*a_x + b_x, '--')
#plt.plot(tgrid, x_opt[1], '-')
ax1 = ax.twinx()
ax1.step(tgrid, np.append(np.nan, u_opt[0]*u_nom[0]), '-.', color="k")
#ax.xlabel('t')
#plt.legend(['x1','u'])
plt.grid()
plt.show()
