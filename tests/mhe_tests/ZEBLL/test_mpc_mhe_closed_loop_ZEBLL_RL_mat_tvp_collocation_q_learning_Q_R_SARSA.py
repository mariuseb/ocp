...
if __name__ == "__main__":
    ...
    dim_P0 = mpc.n_p + mpc.n_x
    # mhe settings:
    P0 = ca.DM.eye(dim_P0)
    
    #for i in range(dim_P0):
    #    for j in range(dim_P0):
    #        if (i != j):
    #            P0[i,j] = 0.01
    # P0 = ca.DM.zeros(7)
    ...
    for k in range(K):
        ...
        # if param scale has changed, re-init mpc scheme
        if not all(
                mpc.get_scale(np.array(params).flatten())
                ==
                np.array(mpc.p_nom).flatten()
                ):
            # new mhe object:
            mpc = MPC(config=mpc_cfg, param_guess=params,
                        **deepcopy(kwargs))
        ...
        if random.random() < epsilon:
            u["phi_h"] = random.random()*5000
            # simulate anew to get x0, mpc prediction will not be correct
            x0 = mpc.integrator.one_sample(x0=x_prev,
                                           z=0,
                                           u=u.values,
                                           p=params,
                                           w=0,
                                           r=data[mpc.r_names].iloc[0].values)["xf"]
        ...
        s_act = max(max(lbx_prev[0] - y_prev, 0), max(y_prev - ubx_prev[0], 0))
        #if s_act > 0:
        #    print(s_act)
        # TODO: modularize:
        #l_k = 0.5*(sol_mpc["phi_h"].iloc[0]/mpc.u_nom)**2 + \
        #      1E8*sol_mpc["s1"].iloc[0]**2
        #L_k = 0.5*(u_meas/mpc.u_nom)**2 + \
        #      1E8*s_act**2
        l_k = 0.5*(sol_mpc["phi_h"].iloc[0]/mpc.u_nom)**2
        #L_k = 0.5*(u_meas/mpc.u_nom)**2 + 1e3*s_act**2
        L_k = 0.5*(u_meas/mpc.u_nom)**2 + 1e2*s_act**2
        ...
        if k >= (mhe.N - 1):
            ...
            try:
                ...
                del_theta, del_Q, del_R = rl.step(mpc,
                                    mhe,
                                    costate_prior,
                                    Q,
                                    R,
                ...
                rl = Qlearning(mpc, mhe, **{"gamma": 0.99, "alpha": 0.10})
                b = 1

                del_P0_accum = np.zeros((P0.shape[0], P0.shape[1]))
                del_R_accum = np.zeros((R.shape[0], R.shape[1]))
                del_Q_accum = np.zeros((Q.shape[0], Q.shape[1]))
            ...
            # filter delta, avoid extreme steps:
            #del_P0_arr = np.array(del_P0)
                # clip below zero:
            
            #if (abs(del_P0_arr) > 1).any():
                #
            #    ndx = np.where(abs(del_P0_arr) > 1)
            #    del_P0_arr[ndx] = 0
            ...
            if b == B:  # update, reset del_P0_accum
                ...
                # check before adding if result is below zero:
                # if so, then reject step in that parameter:
                # (reasoning: std. dev. cannot become negative)
                res = np.array(P0 + del_P0_accum)
                if (np.diag(res) <= 0).any():
                    #ndx = np.where(res <= 0)
                    #del_P0_accum[ndx] = 0
                    del_P0_accum = np.zeros((P0.shape[0], P0.shape[1]))
                
                # new rule: if new Q not positive def, reject parameter update.
                if not ca.det(Q + del_Q_accum) > 0:
                    del_Q_accum = np.zeros((Q.shape[0], Q.shape[1]))    
                    
                if not ca.det(R + del_R_accum) > 0:
                    del_R_accum = np.zeros((R.shape[0], R.shape[1]))    
                    
                """
                res = np.array(Q + del_Q_accum)
                if (np.diag(res) <= 0).any():
                        ndx = np.where(res <= 0)
                    del_Q_accum[ndx] = 0
                
                
                res = np.array(R + del_R_accum)
                if (np.diag(res) <= 0).any():
                    ndx = np.where(res <= 0)
                    del_R_accum[ndx] = 0
                """
    ...
    true_params = ca.DM([2.00107, 12.2739, 0.412547, 2.05257, 4.40102])
    ...
    ax[0].legend([r"$C_i  \enspace  [\frac{kWh}{K}]$"])
    ax[1].legend([r"$C_e \enspace  [\frac{kWh}{K}]$"])
    ax[2].legend([r"$R_{ia} \enspace [\frac{K}{kW}]$"])
    ax[3].legend([r"$R_{ea} \enspace [\frac{K}{kW}]$"])
    ax[4].legend([r"$A_w \enspace [m^2]$"])
