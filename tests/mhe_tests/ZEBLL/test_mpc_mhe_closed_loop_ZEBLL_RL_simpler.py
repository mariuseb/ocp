...
if __name__ == "__main__":
    ...
    for k in range(K):
        ...
        if k >= (mhe.N + 1):
            #diff_Q = gamma*raw_sol_RL["f"] - raw_sol["f"]
            diff_Q = gamma*mhe_sols[k-1]["f"] - mhe_sols[k-2]["f"]
            L_k = mhe.eval_res_obj(sol_mhe, params, R, Q)
            Ls[k] = L_k
            alpha_delta_k = diff_Q*L_k
            alpha_delta_grad_Q_k = alpha_delta_k*np.diag(np.append(jac_p, jac_x))
            gains[k] = alpha_delta_grad_Q_k
            P0 += alpha_delta_grad_Q_k
        ...
        if k >= (mhe.N - 1):
            ...
            try:  
                ...
                mhe_sols[k] = raw_mhe
                jac_p, fisher_p, jac_x = mhe.get_covar_p(params/mhe.p_nom, R, Q)
                #jac_x, fisher_x = mhe.get_covar_x(params/mhe.p_nom, R, Q)
                #L_k = mhe.eval_res_obj(sol_mhe, params, R, Q)
                #covar_x, fisher_x = mhe.get_covar_x(params/mhe.p_nom, R, Q)
                #if params["Ce"] > 5E7:
                #    print(k)
                
                # new reward, actual residual:
                #L_k = 
                
                #if k == 50:
                #    print(mhe_sols)
                #    for k, v in mhe_sols.items():
                #        print(k)
                #        print(v["f"])
