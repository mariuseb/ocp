...
if __name__ == "__main__":
    ...
    mpc_cfg = os.path.join("mpc_configs", "2R2C_PI_MPC_1meas_Ai.json")
    ...
    mhe_cfg = os.path.join("mhe_configs", "3R2C_LimPID_bestest_air_simple_PID_MHE.json")
    ...
    params = ca.DM([0.01,0.001,1E6,5E6,5,500,0.1,120])
    ...
    x0 = x_N = np.array([294.15, 288.15, 0])
    ...
    for k in range(K):
        ...
        if k >= (mhe.N - 1):
            ...
            #x0 = np.append(y_meas[0], x0[1:])
            x0 = np.append(y_meas[:2], sol["E"].iloc[1])
