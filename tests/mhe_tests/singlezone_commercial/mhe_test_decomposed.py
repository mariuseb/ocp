...
if __name__ == "__main__":
    ...
    mhe_cfg = os.path.join("mhe_configs", "3R2C_MHE_simpler.json")
    ...
    MHE params:
    """
    P0 = ca.DM.eye(mpc.n_p + mpc.n_x + mpc.n_z)
    ...
    ekf.df.loc[0] = np.concatenate([x0_env, x0_hvac])
    ...
    for k in range(K):
        ...
        z_model.loc[k*mpc_hvac.dt, mpc.u_names] = sol_env.loc[0, mpc.u_names]
        z_model.loc[k*mpc_hvac.dt, mpc_hvac.z_names] = sol_hvac.loc[0, mpc_hvac.z_names]
        x_model.loc[(k+1)*mpc_hvac.dt, mpc.x_names] = sol_env.loc[1, mpc.x_names]
        x_model.loc[(k+1)*mpc_hvac.dt, mpc_hvac.x_names] = sol_hvac.loc[1, mpc_hvac.x_names]
        ...
        if k >= (mhe.N - 2):
            ...
            y_data["Prad_to_env"] = z_model["Prad_to_env_act"]
            y_data["y1"] = y_data.Ti
            ...
            x_guess = ekf.df[["Ti", "Te"]][-mhe.N:].values.flatten()
            ...
            """
            Consider the simplified case with
            no prior on z:
            """
            #P0[(mhe.n_p + mhe.n_z):, (mhe.n_p + mhe.n_z):] = 0
            ...
            params_env = _params.values
            #x0 = sol_mhe.iloc[-1][mhe.x_names].values
            x0_env = sol_mhe.iloc[-1][mhe.x_names].values
