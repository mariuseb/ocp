...
from ocp.rl import Qlearning, PolicyGradient
import os
import scipy
...
if __name__ == "__main__":
    ...
    # scaling -> should be a shift:
    # e.g., temperature: rarely below
    # 250 degrees K
    kwargs = {
        "x_nom": 300,
        "u_nom": 5000,
    ...
    for k in range(K):
        ...
        if k >= (mhe.N - 1):
            ...
            try:
                ...
                rl = PolicyGradient(mpc, mhe, **{"gamma": 0.99, "alpha": 0.03})
                b = 1
            ...
            #del_P0_accum += del_P0   
            del_P0_accum[b] = del_P0
            if b == B:  # update, reset del_P0_accum
                #P0 += del_P0_accum
                
                theta_step = sum(del_P0_accum.values())/B
