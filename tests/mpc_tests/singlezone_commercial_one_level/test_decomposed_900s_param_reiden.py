...
if __name__ == "__main__":
    ...
    mhe_cfg = os.path.join("mhe_configs", "4R3C_no_slack.json")
    ...
    for k in range(K):
        ...
        if k % MHE_start == 0 and k != 0:
            ...
            y_data["y1"] = y_data.Ti
            y_data["P_to_rad"] = z_model.P_to_rad_act[-MHE_start-1:].shift(-1).values
            y_data = y_data.fillna(0)
            y_data = y_data.loc[:,~y_data.columns.duplicated()].copy()
            
            x_guess = ekf_env.df[["Ti", "Te", "Trad"]].iloc[-MHE_start-1:].values
            ...
            #env_params = ["Rie", "Rea", "Ria", "Ci", "Ce", "Ai", "Rirad", "Crad"]
