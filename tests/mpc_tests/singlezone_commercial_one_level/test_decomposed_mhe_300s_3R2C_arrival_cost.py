...
if __name__ == "__main__":
    ...
    #mhe_cfg = os.path.join("mhe_configs", "6R4C_no_slack_power.json")
    mhe_cfg = os.path.join("mhe_configs", "3R2C.json")
    ...
    #params_env = pd.read_csv("envelope_model_latest_4R3C_rad_flo_non_shifted_15min.csv", index_col=0)
    #params_env = pd.read_csv("envelope_model_latest_4R3C_rad_flo_shifted_15min.csv", index_col=0)
    params_env = pd.read_csv("envelope_model_latest_4R3C_rad_free_params.csv", index_col=0)
    fisher_env = pd.read_csv("envelope_model_latest_4R3C_rad_free_fisher.csv", index_col=0)
    params_hvac.loc["Crad"] = params_env.loc["Crad"]
    params_hvac.loc["Rirad"] = params_env.loc["Rirad"] #*2.5
    params_hvac.loc["Tret_offset"] = 1
    params = pd.concat([params_hvac, params_env])
    params = params[~params.index.duplicated(keep='first')]
    _params = params.copy()
    params = params.to_dict()["0"]
    #params["Crad"] = params["Crad"]/2
    ...
    """
    Fill P0 with values from parameter estimation 
    for env parameters, rest can be zero as
    bounds are tight.
    """
    env_params = ["Rie", "Rea", "Ria", "rho_int", "Ci", "Ce", "Ai"]
    locs = []
    #for name in ret_params:
    for name in env_params:
        loc = _params.index.get_loc(name)
        P0[loc, loc] = fisher_env.loc[name, name]
    ...
    for k in range(K):
        ...
        try:
            P0_x_z = ekf_env.P_aprioris[k - mhe.N + 1]
            #P0_x_z = ekf.P[k]
            # filter:
        ...
        if k >= MHE_start:
            ...
            y_data[["rad_flo", "rad_val", "Prad"]] = y_data[["rad_flo", "rad_val", "Prad"]].shift(-1) 
            y_data["y1"] = y_data.Ti
            y_data["y2"] = y_data.Prad
            #y_data["P_to_rad"] = z_model.P_to_rad_act.shift(-1).values
            y_data["Prad_to_env"] = z_model.Prad_to_env_act[-MHE_start-1:].shift(-1).values
            #y_data["rad_flo"] = z_model.rad_flo.shift(-1).values
            #y_data["P_to_rad"] = z_model.P_to_rad_act.shift(-1).values
            ...
            if k == MHE_start:
                ...
                z_N = ekf_env.df[mhe.z_names].iloc[-mhe.N].values
            ...
            x_guess = ekf_env.df[["Ti", "Te"]][-mhe.N:].values.T
                    
            _Q = ca.DM.eye(2)
            _R = ca.DM.eye(1)
            #_R[0,0] = 1/np.var(y_data["y1"])
            #_R[1,1] = 1/np.var(y_data["y2"])
            
            # set state covariance:
            P0[mhe.n_p:, mhe.n_p:] = np.linalg.inv(P0_x_z[0:2,0:2])
            
            params_lb = 1*params_env
            params_ub = 1*params_env
            
            #env_params = ["n1", "rad_flo_nom", "Rie", "Rea", "Ria", "Ci", "Ce", "Ai", "Rirad", "Crad"]
            env_params = ["Rie", "Rea", "Ria", "rho_int", "Ci", "Ce", "Ai"]
            #env_params = ["Rie", "Ria", "Rea", "rho_int", "Ci", "Ce", "Ai", "Cret", "Rrr", "Crad", "Rirad"]
            #env_params = ["Rie", "Ria", "rho_int", "Ci", "Ai"]
            #ret_params = ["Cret", "Rrr"]
            locs = []
            #for name in ret_params:
            for name in env_params:
    ...
    Parameters:
    """
    fig, axes = plt.subplots(len(env_params),1, sharex=True)
    for i, param in enumerate(env_params):
        axes[i].plot(mhe.df[param], color="k", drawstyle="steps-post")
        axes[i].legend([param])
    plt.show()
    ...
    """
    z_model_cop = z_model.copy()
    x_model_cop = x_model.copy()
    z_model_cop["Prad_to_env"] = z_model_cop["Prad_to_env_act"]
    #z_model = z_model[:-1]
    ...
    z_names = z_model.columns
    z_model_cop.index = res.index
    
    for name, ax in zip(z_names, axes.flatten()):
        try:
            z_model_cop[name].plot(ax=ax, color="r", linewidth=0.75, drawstyle="steps-post")
    ...
    ax = z_model_cop["Prad"].plot(color="r", linewidth=0.75, drawstyle="steps-post")
    ...
    res_x = res
    #res_x = res_x.iloc[:-1]
    x_model_cop.index = res_x.index
    
    for name, ax in zip(x_names, axes.flatten()):
        try:
            x_model_cop[name].plot(ax=ax, color="r", linewidth=0.75, drawstyle="steps-post")
    ...
    #axes[1].plot(res.phi_s, drawstyle="steps-post")
    fig.tight_layout()
    plt.show()
    print(res)