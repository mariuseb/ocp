...
if __name__ == "__main__":
    ...
    mhe_cfg = os.path.join("mhe_configs", "6R4C_power.json")
    ...
    for k in range(K):
        ...
        if k >= MHE_start:
            ...
            x_guess = ekf_env.df[["Ti", "Te","Trad","Tret"]][-mhe.N:].values.T
            ...
            locs = []
            for name in rad_params:
                locs.append(_params.index.get_loc(name))
            params_lb[locs] = 0.5
            params_ub[locs] = 3
            
            rad_params = ["rad_flo_nom"]
    ...
    """
    z_model["Prad_to_env"] = z_model["Prad_to_env_act"]
