...
if __name__ == "__main__":
    ...
    mpc_cfg = os.path.join("mpc_configs", "4R4C_MPC_one_level.json")
    mpc_env_cfg = os.path.join("mpc_configs", "3R2C_MPC_simpler_simpler.json")
    mpc_hvac_cfg = os.path.join("mpc_configs", "HVAC_MPC_300s.json")
    ...
    for k in range(K):
        lbx, ubx, _ = bounds.get_bounds(k, mpc.N)
        """
        Run whole problem:
        """
        sol, u, x0 = mpc.solve(
        ...
        sol.index = data.index[0:len(sol)]
        ...
        data_hvac["Prad_to_env_ideal"] = sol_env["Prad_to_env"].iloc[0:mpc_hvac.N]
        ...
        x_model.loc[k+1, mpc.x_names] = sol.iloc[1][mpc.x_names]
        z_model.loc[k, mpc.z_names] = sol.iloc[0][mpc.z_names]
        ...
        Estimate state:
        """
        u_model = sol[ekf_env.dae.u].iloc[0].values
