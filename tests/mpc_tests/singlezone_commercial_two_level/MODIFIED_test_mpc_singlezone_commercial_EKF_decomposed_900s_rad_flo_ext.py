...
if __name__ == "__main__":
    ...
    mpc_env_short_cfg = os.path.join("mpc_configs", "3R2C_MPC_simpler_simpler_short.json")
    ...
    env_index = [mpc.dt*n for n in range(mpc.N)]
    env_I = mpc_env_short.integrator.one_sample
    ...
    for k in range(K):
        ...
        if k % 3 == 0:
            ...
            lbx_env = np.array([290.15,280]*(mpc_env.N-1))
            ubx_env = np.array([296.15,340]*(mpc_env.N-1))
            x0_env_prev = x0_env
            ...
            dist_hvac = sol_env[["Ti"]].loc[0:mpc_hvac.N*mpc_hvac.dt]
            dist_hvac = dist_hvac.reindex(data_hvac.index)
            
            """
            Simulate with Prad_to_env to fill holes in Ti:
            """
            #dist_hvac = dist_hvac.ffill()
            xs = []
            x0 = x0_env_prev
            for i in range(3):
                x0 = env_I(x0=x0,u=u_env.values,p=params_env,r=sol_env[mpc_env.r_names].iloc[0].values)["xf"]
                dist_hvac.loc[(i+1)*mpc_hvac.dt, "Ti"] = x0[0]
            
            
            #data_hvac["Ti"] = sol_env.Ti.loc[data_hvac.index]
            #data_hvac["Trad_ENV"] = sol_env.Trad_ENV.loc[data_hvac.index]
            data_hvac[dist_hvac.columns] = dist_hvac
            """
            Interpolate between 0, -1 for Ti:
            """
            #data_hvac["Ti"] = data_hvac["Ti"].interpolate(method="linear")
            #params_hvac[-2] = sol_env.Prad_to_env.iloc[1].astype(float)
            ...
            x_model.loc[int(k/3) + 1, mpc.x_names] = sol_env.iloc[1][mpc.x_names]
            x_model.loc[int(k/3) + 1, mpc_hvac.x_names] = sol_hvac.iloc[1][mpc_hvac.x_names]
