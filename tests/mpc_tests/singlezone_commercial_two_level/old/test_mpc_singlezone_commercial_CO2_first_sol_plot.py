...
if __name__ == "__main__":
    ...
    for k in range(K):
        ...
        """
        sol.to_csv("test_data.csv")
        sol.index *= mpc.dt
        sol.index = pd.TimedeltaIndex(data=sol.index, unit="s")
        ...
        ax = axes[1]
        sol.CO2_in.plot(ax=ax, color="g", drawstyle="steps-post", linewidth=0.75)
        ax1 = ax.twinx()
        sol.ahu_pump_sup.plot(ax=ax1, color="y", drawstyle="steps-post", linewidth=0.75)
        ax1.set_ylabel("Normalized fan speed [-]")
        ax.set_ylabel("CO2-level [ppm]")
        ax.legend(["CO2-level"], loc="upper left")
        ax1.legend(["Fan speed"])
