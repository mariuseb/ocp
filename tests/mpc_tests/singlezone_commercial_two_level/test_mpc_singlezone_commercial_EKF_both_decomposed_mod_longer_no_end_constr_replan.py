...
if __name__ == "__main__":
    ...
    mpc_hvac_cfg = os.path.join("mpc_configs", "HVAC_MPC_no_CO2_adj_Tsup_300s_mod_longer_no_end.json")
    ...
    for k in range(K):
        ...
        if k % 3 == 0:
            ...
            data_hvac.loc[0, "Ti"] = x0_env[0].astype(float)
            data_hvac.loc[3*mpc_hvac.dt, "Ti"] = sol_env.Ti[1].astype(float)
            data_hvac.loc[(mpc_hvac.N-1)*mpc_hvac.dt, "Ti"] = sol_env.Ti[2].astype(float)
            """
            Interpolate between 0, -1 for Ti:
            """
            data_hvac["Ti"] = data_hvac["Ti"].interpolate(method="linear")
            #data_hvac["Ti"] = data_hvac["Ti"].interpolate(method='polynomial', order=2)
