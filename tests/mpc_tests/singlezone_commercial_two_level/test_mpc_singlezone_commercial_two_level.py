...
if __name__ == "__main__":
    ...
    mpc_cfg = os.path.join("mpc_configs", "3R2C_MPC_CO2_envelope.json")
    ...
    for k in range(K):
        ...
        sol.Pvent.plot(color="y", ax=ax)
        sol.Pvent_to_env.plot(color="b", ax=ax)
        ax1 = ax.twinx()
        sol.Tsup_air.plot(ax=ax1, color="k")
        sol.Ti.plot(ax=ax1, color="g")
        ax.legend()
        plt.show()
        """
        Solve for valve openings:
        """
