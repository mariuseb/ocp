...
if __name__ == "__main__":
    ...
    mpc_env_cfg = os.path.join("mpc_configs", "3R2C_MPC_min_energy_900s_mod.json")
    ...
    ekf_env_cfg = os.path.join("ekf_configs", "3R2C_simpler_simpler_EKF_900s_mod.json")
    ...
    Keep history:
    """
    z_model = pd.DataFrame(columns=mpc.z_names + mpc_hvac.z_names)
    ...
    for k in range(K):
        ...
        if k % 3 == 0:
            ...
            Set Ti, Prad_to_env
            """
            x_model.loc[k+3, mpc.x_names] = sol_env.iloc[1][mpc.x_names]
        ...
        #data_hvac["Ti_ENV"] = sol_env.Ti_ENV.loc[data_hvac.index]
    
        data_hvac["Te"] = sol_env.Te.iloc[0]
        data_hvac["Ti_ENV"] = sol_env.Ti_ENV.iloc[1]
        ...
        z_model.loc[k, z_model.columns] = sol_hvac.loc[0, z_model.columns]
        ...
        u_model = [sol_hvac[name].iloc[0] for name in ekf_hvac.dae.u]
        ...
        """
        Estimate Prad_to_env from filter.
        """
