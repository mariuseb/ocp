...
if __name__ == "__main__":
    ...
    mpc_env_cfg = os.path.join("mpc_configs", "4R4C_MPC_min_energy_900s.json")
    ...
    ekf_env_cfg = os.path.join("ekf_configs", "4R4C_simpler_simpler_EKF.json")
    mhe_cfg = os.path.join("mhe_configs", "4R4C_MHE_900s.json")
    ...
    kwargs["z_nom_b"] = 0
    """
    est_p_nom = np.array(list(mpc_env.p_nom))
    kwargs = {
        "x_nom": 1,
        "x_nom_b": 0,
        "z_nom": 1,
    ...
    Keep history:
    """
    z_model = pd.DataFrame(columns=list(set(mpc.u_names)))
    x_model = pd.DataFrame(columns=mpc.x_names)
    
    x_model.loc[0, mpc.x_names] = x0_env[:mpc.n_x]
    ...
    P0 = ca.DM.eye(mhe.n_p + mhe.n_x)*0
    #P0[3,3] = 1E2
    P0[6,6] = 1E2
    #P0[9,9] = 1E2
    P0[13,13] = 1E4
    ...
    est_params = mhe.p0
    params_lb = 0.1*est_params
    params_ub = 10*est_params
    params_lb[-2] = 0.3
    params_ub[-2] = 3
    params_lb[-3] = 5
    params_ub[-3] = 10
    for k in range(K):
        ...
        """
        z_model.loc[k, mpc.z_names] = sol_env.iloc[0][mpc.z_names]
        # forward set:
        z_model.loc[k+1, mpc.z_names] = np.nan
        ...
        u_env["ahu_pump_sup"] = 0.03
        u_env["ahu_pump_ret"] = 0.03
        """
        ...
        Advance:
        """
        data, y_meas, u_meas = boptest.evolve(u=u_env,
                                            y_as_array=False,
                                            u_as_array=False)
        ...
        if k >= (mhe.N - 2):
            ...
            if k == (mhe.N - 2):
                ...
                x_N = ekf_env.df[mhe.x_names].iloc[-mhe.N+2].values
            else:
                # use EKF smoothing:
                
                #x_N = sol_mhe.iloc[1][mhe.x_names].values
                x_N = ekf_env.df[mhe.x_names].iloc[-mhe.N+2+k].values
                #x_N = x0 = x10
            ...
            if k < (mhe.N - 1):
                x_guess = ekf_env.df[mhe.x_names].sort_index()[-mhe.N:].values.flatten()
            ...
            __P0x = ca.inv(ekf_env.P_aprioris[k - mhe.N + 1])
            P0[
