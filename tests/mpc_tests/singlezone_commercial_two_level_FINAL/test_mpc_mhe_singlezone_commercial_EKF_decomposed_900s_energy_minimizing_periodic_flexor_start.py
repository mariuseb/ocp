...
if __name__ == "__main__":
    ...
    #params_lb = 1E-2*params_env
    #params_ub = 1E2*params_env
    P0 = ca.DM.eye(mpc_env.n_p + mpc_env.n_x)*1
    P0[19,19] = 0
    P0[20,20] = 0
    P0[21,21] = 0
    P0[3,3] = 1E2
    ...
    for k in range(K):
        ...
        if k >= (mhe.N - 1):
            ...
            if k < (2*mhe.N - 1):
                ...
                x_guess = mhe.df[["Ti", "Te","Trad_ENV"]][-mhe.N:].shift(-1).ffill().values.T
            ...
            if not all(
                ...
                mhe.df = df
            """
            #params_env.loc[params_env.index[params_env.round(7) == 0].tolist()] = 1e-3 
            if params_env.loc["Ai"].round(2) == 0:
                params_env.loc["Ai"] = 0.1
            if params_env.loc["rho_int"].round(2) == 0:
                params_env.loc["rho_int"] = 0.1
            params_lb = lb_mult*params_env
            params_ub = ub_mult*params_env
            
            if k == 6*24*4:
                print(P0)
                P0 = ca.DM.eye(mpc_env.n_p + mpc_env.n_x)*0
                P0[19,19] = 0
                P0[20,20] = 0
                P0[21,21] = 0
                lb_mult = 1
            ...
            # move bounds (not sure if needed):
            #print(params)
