...
if __name__ == "__main__":
    ...
    mpc_env_cfg = os.path.join("mpc_configs", "3R3C_MPC_ref_tracking_900s.json")
    #mpc_hvac_cfg = os.path.join("mpc_configs", "HVAC_MPC_si#mpler_300s.json")
    mpc_hvac_cfg = os.path.join("mpc_configs", "HVAC_MPC_simpler_60s_ref_tracking.json")
    boptest_cfg = os.path.join(bop_config_base, "ZEBLL_config.json")
    ekf_env_cfg = os.path.join("ekf_configs", "3R3C_simpler_simpler_EKF_900s.json")
    ...
    for k in range(K):
        ...
        if k % 15 == 0:
            ...
            dist_hvac = sol_env[["Ti", "Trad_ENV"]].loc[0:mpc_hvac.N*mpc_hvac.dt]
            dist_hvac = dist_hvac.reindex(data_hvac.index)
            dist_hvac = dist_hvac.bfill()
            ...
            x_model.loc[int(k/15) + 1, mpc.x_names] = sol_env.iloc[1][mpc.x_names]
            x_model.loc[int(k/15) + 1, mpc_hvac.x_names] = sol_hvac.iloc[1][mpc_hvac.x_names]
