...
if __name__ == "__main__":
    ...
    us = pd.DataFrame(columns=["calc", "act"])

    for k in range(K):
        
        lbx, ubx, ref = bounds.get_bounds(k, mpc.N)
        
        # TODO: parsing of objective
        sol, u_model, x0  = mpc.solve(
                               data[0:mpc.N],
                               x0=x0,
                               lbx=lbx,
                               ubx=ubx,
        ...
        """
        For calculation of u in terms of Tsup,
        take u/u_max * delta_T.
        """
        
        u = {
             "oveTSetSup_u": float(((u_model/5000)*50 + 293.15).iloc[0]),
             "oveTSetSup_activate": 1
            }    
    
        # "correct" value, according to simplified NL-model:
        
        time.loc[k] = mpc.solver.stats()["t_wall_total"]
        data, y_meas, u_meas = boptest.evolve(u=u)
        us.loc[k, "calc"] = u_model.iloc[0]
    ...
    ax = us.act.plot()
    us.calc.plot(ax=ax, linestyle="dashed")
    ax.legend()
    plt.show()
