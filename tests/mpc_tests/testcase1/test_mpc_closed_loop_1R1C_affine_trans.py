...
if __name__ == "__main__":
    ...
    #p_val = np.array([1, 1, 0, 0, 0, 0])
    p_val = np.array([1, 1, 1, 1, 0, 0])
    p_val = np.array([10, 1, 0, 0, 0, 0])
    p_val = np.array([0.01, 1, 0, 0, 0, 0])
    ...
    for k in range(K):
        ...
        # keep lambda p:
        #lam_df_func.loc[k] = np.array(V_p(p=p_val, x=raw_sol["x"])["V_p"]).flatten()
        # back-shift index:
        data.index -= mpc.dt
        # bootstrap, solve MPC wither model prediction of next step:
        ...
        if b < B: # batch learning, accumulate:
            ...
            TD_0 = (R_k + diff_Q)
            # keep:
            df.loc[k, "TD-0"] = float(TD_0)
            
            delta_theta += raw_sol_qrqp["lam_p"]
            delta_p += raw_sol_qrqp["lam_x"][p_start:p_stop]*mpc.scale
            TD_0_theta += alpha*delta_theta*TD_0
            TD_0_p += alpha*delta_p*TD_0
