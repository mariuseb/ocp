...
class LtiSystem(gym.Env[npt.NDArray[np.floating], float]):
    ...
    def set_state(self):
        self.x = np.asarray([295.65 ]).reshape(self.nx, 1)
        self.k = 0
        return self.x, {}
...
if __name__ == "__main__":
    ...
    n = ceil(sqrt(mpc.nP))
    fig, axes = plt.subplots(n,n)
    cols = mpc.learning_module.grad_a_history.columns
    for i, ax in enumerate(axes.flatten()):
        try:
            col = cols[i]
            ser = mpc.learning_module.grad_a_history[col]
            ser.plot(ax=ax)
            ax.set_title(cols[i])
        except IndexError:
            pass
