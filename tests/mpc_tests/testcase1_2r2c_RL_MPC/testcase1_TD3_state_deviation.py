...
if __name__ == "__main__":
    ...
    mpc_cfg = os.path.join("1R1C_MPC_REINFORCE_t_external_input.json")
    # perturb = 3
    #perturb = 0.33
    perturb = 1.5
    days = 5
    ...
    all_data["time"] = all_data.index
    all_data["t"] = all_data.time.apply(lambda x: x % 96)
    df_opt = pd.read_csv("optimal_u_20_days.csv", index_col=0)
    df_res = pd.read_csv("res_df.csv", index_col=0)
    
    for k in range(K):
        policy_params[5:16] = df_res["~Ti"][k:(k+mpc.N-1)].values
        """
        See if same effect persists with just one bias:
        """
        policy_params[6:16] = 0
    ...
    df.to_csv("res_df.csv", index=True)
