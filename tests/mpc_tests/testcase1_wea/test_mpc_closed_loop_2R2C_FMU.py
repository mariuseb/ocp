...
if __name__ == "__main__":
    ...
    mpc_cfg = os.path.join("mpc_configs", "2R2C_FMU_MPC.json")
    #mpc_cfg = os.path.join("mpc_configs", "2R2C_MPC.json")
    ...
    #ekf_cfg = os.path.join("ekf_configs", "2R2C_EKF.json")
    ekf_cfg = os.path.join("ekf_configs", "2R2C_EKF_reverse.json")
    ...
    #params = ca.DM([0.001,0.01,1E6,5E6,1E-5])
    params = ca.DM([0.001,0.01,1E6,5E6,5])
    ...
    dae = ca.DaeBuilder('pi', 'FMUs/Simple2R2CSimple/Simple2R2CSimple.fmutmp')
    dae.disp(True)
    aux = dae.p()
    name_in = ['x','u']
    name_out = ['ode']
    ...
    ode = dae.create('ode', name_in, name_out, {"verbose": False, "aux": aux,
                                              "parallelization": "serial"})
    functions["pi_ode"] = ode
    #functions["pi_y"] = pi_y
    ...
    fig.tight_layout()
    plt.show()    
    ax = data.Ti.plot(color="k")
    ekf.df.Ti.plot(ax=ax, color="r", linestyle="dashed")
    plt.show()
    print(data)
