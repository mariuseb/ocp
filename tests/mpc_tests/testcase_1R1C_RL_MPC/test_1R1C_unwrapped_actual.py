...
if __name__ == "__main__":
    ...
    buffer = pd.DataFrame(
        columns=["r", "Q(s_k, a_k)", "V(s_k+1)", "td-error", "P[0,0]", "P[1,1]", "P[2,2]"] + dQ_cols
    )
    ...
    for k in range(K):
        ...
        if coord.controller.re_estimation_clause(coord.controller.i-1):
            ...
            buffer.loc[k, "r"] = r
            # store Q(s,a):
            buffer.loc[k, "Q(s_k, a_k)"] = Q_k = float(raw_sol["f"])
            # store for V(s_k+1):
            try:
                buffer.loc[k, "V(s_k+1)"] = V_k_plus = float(raw_sol_prev["f"]) + r
