...
if __name__ == "__main__":
    ...
    # config:
    mpc_env_cfg = os.path.join("mpc_configs", "2R2C_MPC_env_energy.json")
    mpc_rad_cfg = os.path.join("mpc_configs", "3R3C_MPC_rad_energy.json")
    boptest_cfg = os.path.join(bop_config_base, "ZEBLL_config.json")
    ekf_cfg = os.path.join("ekf_configs", "5R5C_EKF_1meas.json")
    ...
    kwargs_rad = {
        ...
        "z_nom": [1,12,1E4,1E4,1E4],
        "z_nom_b": [0,289.15,0,0,0],
        "r_nom": [12,1E4,1E4,12],
    ...
    x0_rad = np.array([293.15,293.15,293.15])
    ...
    for k in range(K):
        ...
        #sol_env["Ti"].loc[sol_env["Prad_to_env"] < 600] = np.nan
        ##############################
        data_rad["Prad_to_env_ENV"] = sol_env["Prad_to_env"][0:mpc_rad.N].values
        data_rad["Ti_ENV"] = sol_env["Ti"][0:mpc_rad.N].values
        lbx_rad = np.array([280,280,280]*(mpc_rad.N-1))
        ubx_rad = np.array([340,340,340]*(mpc_rad.N-1))
