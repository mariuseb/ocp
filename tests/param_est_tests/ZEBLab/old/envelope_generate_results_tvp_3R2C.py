...
params = pd.DataFrame(pd.read_csv("results/params_envelope_tvp_3R2C.csv", index_col=0))
# to get estimated x0, read trajectory:
sol = pd.read_csv("results/tvp_in_training_traj.csv", index_col=0)
...
result_gen = ResultGenerator(
                             config="configs/3R2C_det_vent_207_cond_weekend_full_tvp_vent_cond.json",
                             params=params,
                             dt=3600
                            )
...
p_base = pd.read_csv("results/tvp_params_base_3R2C.csv", index_col=0).values.flatten()
p_mod = pd.read_csv("results/tvp_params_mod_3R2C.csv", index_col=0).values.flatten()
...
result_gen.make_journal_plot(
                             y_data,
                             x0, 
                             "tvp_journal_plot_training_3R2C"
                             )
result_gen.plot_residual_dist(
                              y_data,
                             "tvp_residual_plot_training_3R2C"
                             )
...
result_gen.make_journal_plot(
                             y_data,
                             x0, 
                             "tvp_journal_plot_validation_3R2C"
                             )
result_gen.plot_residual_dist(
                              y_data,
                             "tvp_residual_plot_validation_3R2C"
                             )
