...
if __name__ == "__main__":
    ...
    cfg_path = os.path.join("configs", "2R2C_det_vent_tvp_heating_cond.json")
    ...
    start = pd.Timestamp("2023-10-31 00:00")
    stop = pd.Timestamp("2023-11-20 00:00")
    ...
    with ParameterEstimation(config=cfg_path,
        ...
        plt.show()
        params.to_csv("results/params_envelope_tvp_2R2C_heating.csv", index=True)
        print(params)
    # dump for plots:
    sol.to_csv("results/tvp_in_training_traj_2R2C_heating.csv", index=True)
    ...
    p_mod[-3:] = [0,0,0]
    z_guess = params[["Rie", "Rea", "Ci", "Ce"]].values
    v = [0]
    N = len(y_data)
    for n in range(N):
    ...
    #plt.show()
    res.to_csv("results/tvp_validation_traj_2R2C_heating.csv", index=True)
    ...
    # set last three to zero:
    p_mod[4:] = 0
    p_base.to_csv("results/tvp_params_base_2R2C_heating.csv", index=True)
    p_mod.to_csv("results/tvp_params_mod_2R2C_heating.csv", index=True)
