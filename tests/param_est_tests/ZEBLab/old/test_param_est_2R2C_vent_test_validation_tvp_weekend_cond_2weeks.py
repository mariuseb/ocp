...
if __name__ == "__main__":
    ...
    cfg_path = os.path.join("configs", "2R2C_det_vent_tvp_weekend_cond.json")
    ...
    with ParameterEstimation(config=cfg_path,
        ...
        plt.show()
        params.to_csv("results/params_envelope_tvp_2R2C_weekend.csv", index=True)
        print(params)
    # dump for plots:
    sol.to_csv("results/tvp_in_training_traj_2R2C_weekend.csv", index=True)
    ...
    #plt.show()
    res.to_csv("results/tvp_validation_traj_2R2C.csv", index=True)
    ...
    # set last three to zero:
    p_mod[4:] = 0
    p_base.to_csv("results/tvp_params_base_2R2C_weekend.csv", index=True)
    p_mod.to_csv("results/tvp_params_mod_2R2C_weekend.csv", index=True)
