...
if __name__ == "__main__":
    ...
    def prepare_data(data):
        ...
        temps_219.plot(ax=ax)
        data.T_sup_219.plot(ax=ax)
        data.T_ret_219.plot(ax=ax)
        ax1 = ax.twinx()
        ...
        ax = axes[1]
        data["val_219"].plot(ax=ax, linewidth=0.5, drawstyle="steps-post", color="b")
        plt.show()
        
        # plot val pos vs. mass flow:
        ax = data["val_219"].plot(linewidth=0.5, drawstyle="steps-post", color="b")
        ax1 = ax.twinx()
        data["V_flow_219"].plot(ax=ax1, linewidth=0.5, drawstyle="steps-post", color="r")
        ax.legend(["val"])
        ax1.legend(["flow"])
        plt.show()
    ...
    xs = np.array([])
    p = params
    p_base = params[["Ce", "Ci", "Rie_b", "Rea"]].values
    p_mod = params[["Ce_a", "Ci_a", "Rie_a", "Rea_a"]].values
    #p_base = params[["Ce", "Ci"]].values
    #p_mod = params[["Ce_a", "Ci_a"]].values
