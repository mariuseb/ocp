...
if __name__ == "__main__":
    ...
    cfg_path = os.path.join("configs", "3R3C_det_tvp_vent_new_interface_alt.json")
    ...
    def prepare_data(data):
        ...
        y_data["y1"] = y_data["Ti"]
        y_data["Time"] = data.index
        
        return y_data
    
    y_data = prepare_data(data)
    y_data["delta_phi_h"] = y_data.phi_h - y_data.phi_h.shift(1)
    y_data["phi_h_rise"] = 0
    y_data["phi_h_fall"] = 0
    non_zero_rise =  y_data["delta_phi_h"][y_data["delta_phi_h"] > 0].astype(bool).astype(int)
    non_zero_fall =  y_data["delta_phi_h"][y_data["delta_phi_h"] < 0].astype(bool).astype(int)
    y_data["phi_h_rise"].loc[non_zero_rise.index] = non_zero_rise
    y_data["phi_h_fall"].loc[non_zero_fall.index] = non_zero_fall
    ...
    with ParameterEstimation(config=cfg_path,
        ...
        """
        Plot also in-training of the 
        non-TVP model.
        """
        non_tvp = pd.read_csv("NL_in_training_traj.csv", index_col=0)
        # reset index to datetime:
        non_tvp.index = y_data.Time
        y_data.index = y_data.Time
        sol.index = y_data.Time
        # plot:
        fig, ax = plt.subplots(1,1)
        sol["Ti"].plot(ax=ax, color="r")        
        non_tvp["Ti"].plot(color="b", linewidth=1, linestyle="dashed", ax=ax)
    ...
    xs = np.array([])
    p = params
    p_base = params[["Ci_b", "Ce_b", "Rie_b"]].values
    p_mod = params[["Ci_a", "Ce_a", "Rie_a"]].values
    ...
    for n in range(N):
        ...
        Check if V_sup_air > 0. 
        If so, then add modifiers.
        ...
        # wdef takes x at current timestep
        # -> need to solve along with ode in irk rootfinder.
        w = wdef(0,x0,0,u,p,0,r,0)
            
        #z_alt = sol[I.dae.z].iloc[n].values
        #zs.loc[n] = [z[0], z_alt[0]]
        x0 = I.one_sample(x0,0,u,p,0,r,w)
    ...
    #ax.legend(["TVP model", "NL model", "measured"])
    ax.legend(["TVP model", "measured"])
    ax.set_ylabel("Temperature [$^\circ$C]")
    fig.tight_layout()
    plt.show()
    print(params)

    acf_tvp = acf(res["Ti"] - y_data.Ti)
    acf_non_tvp = acf(non_tvp["Ti"] - y_data.Ti)
    acf_df = pd.DataFrame(columns=["acf_tvp", "acf_non_tvp"])
    acf_df["acf_tvp"] = acf_tvp
    acf_df["acf_non_tvp"] = acf_non_tvp
