...
if __name__ == "__main__":
    ...
    cfg_path = os.path.join("configs", "valve_ODE_TVP_alg.json")
    ...
    # resample
    normed = (data.delta_m_flow.resample(rule="2min").mean()/data.delta_m_flow.max()).resample("5min").ffill()
    normed[normed < 0] = 0
    normed[normed > 0] = normed[normed > 0].astype(bool).astype(int)
    data["m_flow_rise"] = normed
    normed = (data.delta_u_val.resample(rule="2min").mean()/data.delta_u_val.max()).resample("5min").ffill()
    normed[normed > 0] = 0
    normed[normed < 0] = normed[normed < 0].astype(bool).astype(int)
    ...
    param_guess = np.array([8.26750000e+13, 
                            3.42434833e+02, 
                            1.41798364e+01,
                            5.00054867e+01,
                            6.94493441e-03])
    ...
    ubp[len_p-2] = 7
    """    
    x_guess = data[["u_val_set"]]
    #x_guess = data[["u_val_set", "u_val_set"]]
    #x_guess.columns = ["u_val_set", "tau_fall"]
    #x_guess += 0.003
    #x_guess["tau_fall"] += 1
    #x_guess["tau_fall"] += 300
    x_guess_arr = x_guess.values.flatten()
    lbx = (x_guess*0)
    ubx = (x_guess+1)
    #lbx.iloc[0] = [0,0]
    #ubx.iloc[0] = [0,0]
    lbx = lbx.values.flatten()
    ...
    with ParameterEstimation(config=cfg_path,
        ...
        ax1 = ax.twinx()
        #sol["x_tau_fall"].plot(ax=ax1, color="m", drawstyle="steps-post")
        sol["tau_fall"].plot(ax=ax1, color="y", drawstyle="steps-post")
        plt.show()
        print(params)
