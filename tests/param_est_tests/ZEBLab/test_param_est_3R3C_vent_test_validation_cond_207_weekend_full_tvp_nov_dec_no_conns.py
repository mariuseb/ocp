...
if __name__ == "__main__":
    ...
    cfg_path = os.path.join("configs", "3R3C_det_vent_207_cond_weekend_full_tvp_no_conns.json")
    #data_path = os.path.join("ZEBLab_year_15m_T_last.csv")
    data_path = os.path.join("ZEBLab_month_60m.csv")
    ...
    p_base = params[["Rie","Rea","Rih","Ria","Ci","Ce","Ch","Ai","Ae","alpha_vent"]].values
    p_mod = params[["Rie_a","Rea_a","Rih_a","Ria_a","Ci_a","Ce_a","Ch_a","Ai","Ae","alpha_vent"]].values
    # set last three to zero:
    p_mod[-3:] = [0,0,0]
    z_guess = params[["Ce", "Ci", "Ch", "Rie", "Rea", "Ria", "Rih"]].values
    v = [0]
    N = len(y_data)
    for n in range(N):
    ...
    #plt.show()
    res.to_csv("results/tvp_validation_traj.csv", index=True)
    ...
    # set last three to zero:
    p_mod[-10:] = [0]*10
    p_base.to_csv("results/tvp_params_base.csv", index=True)
    p_mod.to_csv("results/tvp_params_mod.csv", index=True)
