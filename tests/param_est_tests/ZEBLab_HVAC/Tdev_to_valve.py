...
if __name__ == "__main__":
    ...
    cfg_path = os.path.join("configs", "Tdev_to_valve.json")
    ...
    with ParameterEstimation(config=cfg_path,
        ...
        fig, axes = plt.subplots(2,1,sharex=True)
        sol[["E", "Tdev"]].plot(drawstyle="steps-post", ax=axes[0])
        sol[["u_val_cmd", "y2"]].plot(drawstyle="steps-post", ax=axes[1])
        sol[["u_val"]].plot(drawstyle="steps-post", linestyle="dashed", ax=axes[1])
        plt.show()
        print(sol)
