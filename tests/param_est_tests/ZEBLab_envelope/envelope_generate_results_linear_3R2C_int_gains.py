...
params = pd.DataFrame(pd.read_csv("results/params_envelope_3R2C_int_gains.csv", index_col=0))
# to get estimated x0, read trajectory:
sol = pd.read_csv("results/linear_in_training_traj_3R2C_int_gains.csv", index_col=0)
Data = ZEBData("ZEBLab_nov23_1m.csv")
#Data = ZEBData("ZEBLab_2024_1m.csv")
...
result_gen = ResultGenerator(
                             ...
                             config="configs/3R2C_det_vent_int_gains.json",
                             params=params,
                             dt=3600
                            )
...
ekf_config = "configs/ekf_configs/3R2C_envelope_int_gains_EKF.json"
...
result_gen.make_journal_plot(
                             y_data,
                             x0, 
                             "linear_3R2C_int_gains_journal_plot_training"
                             )
result_gen.plot_residual_dist(
                              y_data,
                             "linear_3R2C_int_gains_residual_plot_training"
                             )
...
result_gen.make_journal_plot(
                             y_data,
                             x0, 
                             "linear_3R2C_int_gains_journal_plot_validation"
                             )
result_gen.plot_residual_dist(
                              y_data,
                             "linear_3R2C_int_gains_residual_plot_validation"
                             )
