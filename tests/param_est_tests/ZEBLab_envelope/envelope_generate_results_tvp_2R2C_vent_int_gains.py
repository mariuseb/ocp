...
params = pd.DataFrame(pd.read_csv("results/params_envelope_tvp_2R2C_vent_int_gains.csv", index_col=0))
# to get estimated x0, read trajectory:
sol = pd.read_csv("results/tvp_in_training_traj_2R2C_vent_int_gains.csv", index_col=0)
data_path = os.path.join("ZEBLab_nov23_1m.csv")
zeb_data = ZEBData(data_path)
y_data = zeb_data.get_dataset(
...
result_gen = ResultGenerator(
                             config="configs/2R2C_det_vent_tvp_vent_cond_int_gains.json",
                             params=params,
                             dt=3600
                            )
...
p_base = pd.read_csv("results/tvp_params_base_2R2C_vent_int_gains.csv", index_col=0).values.flatten()
p_mod = pd.read_csv("results/tvp_params_mod_2R2C_vent_int_gains.csv", index_col=0).values.flatten()
...
result_gen.make_journal_plot(
                             y_data,
                             x0, 
                             "tvp_journal_plot_training_2R2C_vent_int_gains"
                             )
res = result_gen.plot_residual_dist(
                                    y_data,
                                    "tvp_residual_plot_training_2R2C_vent_int_gains"
                                    )
"""
...
result_gen.make_journal_plot(
                             y_data,
                             x0, 
                             "tvp_journal_plot_validation_2R2C_vent_int_gains"
                             )
result_gen.plot_residual_dist(
                              y_data,
                             "tvp_residual_plot_validation_2R2C_vent_int_gains"
                             )
