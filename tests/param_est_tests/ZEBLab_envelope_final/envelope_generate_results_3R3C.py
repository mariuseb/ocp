...
params = pd.DataFrame(pd.read_csv("results/params_envelope_3R3C.csv", index_col=0))
# to get estimated x0, read trajectory:
sol = pd.read_csv("results/tvp_in_training_traj_3R3C.csv", index_col=0)
...
#y_data["y1_already"] = y_data_already["y1"]
#y_data["phi_h_already"] = y_data_already["phi_h"]
#y_data["vent_on_already"] = y_data_already["vent_on"]

#y_data = y_data_already
"""
"""
...
result_gen = ResultGenerator(
                             config="configs/3R3C_det_vent_no_slack_int_gains.json",
                             params=params,
                             slack=True,
                             dt=900
                            )
...
#ekf_config = "configs/ekf_configs/3R3C_envelope_EKF.json"
ekf_config = "configs/ekf_configs/3R3C_envelope_EKF_int_gains.json"
...
result_gen.make_journal_plot(
                             y_data,
                             x0, 
                             "journal_plot_training_3R3C_vent"
                             )
plt.show()
res = result_gen.plot_residual_dist(
                                    y_data,
                                    "residual_plot_training_3R3C_vent"
                                    )
plt.show()
...
y_data = Data.get_dataset(
                          start = pd.Timestamp("2024-01-22 00:00"),
...
result_gen.make_journal_plot(
                             y_data,
                             x0, 
                             "journal_plot_validation_2R2C_vent"
                             )
result_gen.plot_residual_dist(
                              y_data,
                             "residual_plot_validation_2R2C_vent"
                             )
