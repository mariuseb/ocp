...
if __name__ == "__main__":
    ...
    cfg_path = os.path.join("configs", "3R3C_det_vent_207_cond_weekend_full_tvp.json")
    sol = pd.read_csv("results/NL_in_training_traj.csv", index_col=0)
    params = pd.DataFrame(pd.read_csv("results/params_envelope_tvp.csv", index_col=0))
    ...
    #plt.show()
    res.to_csv("results/NL_validation_traj.csv", index=True)
    ...
    ekf_base = KalmanBucy(cfg)
    ekf_weeknd = KalmanBucy(cfg)
    ekf_base.set_params(p_base)
    ekf_weeknd.set_params(p_base + p_mod)
    ...
    for n in range(N-1):
        ...
        if y_data["dt_index"].iloc[n].dayofweek > 4: # switch filter
            p = p_base + p_mod
    ...
    #res["v1"][-48:] = 0
    plot_residuals(result, res, "Residuals, validation")
