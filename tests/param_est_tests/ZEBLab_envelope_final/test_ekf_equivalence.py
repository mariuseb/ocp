...
if __name__ == "__main__":
    ...
    cfg_path = os.path.join("configs", "2R2C_int_gains_sep.json")
    ekf_config = "configs/ekf_configs/2R2C_envelope_EKF_int_gains_sep.json"
    ...
    for delta_day in range(1):
        ...
        if not y_data.Ti.isna().any():
            ...
            filtered_map = result_gen.filtered
            ...
            filtered = result_gen.filtered
            
            ax = filtered_map["y_pred"].plot(color="b")
            filtered["y_pred"].plot(color="r", ax=ax)
            ax.legend(["map", "serial"])
            plt.show()
