...
if __name__ == "__main__":
    ...
    kwargs = {
        ...
        "u_nom": [1E3,1E3,1E3,12,10,10,12],
        "u_nom_b": [0,0,0,12,0,0,12],
        "y_nom": [12],
        "y_nom_b": [289.15],
        #"slack": True
        "slack": False
    ...
    with MHE(config=cfg_path,
        ...
        for n in range(len_priors, P0.shape[0]):
            P0[n,n] = 0
        ...
        sol_mhe, params_mhe = mhe.solve(
                                      ...
                                      ubx=ubx,
                                      x_guess=x_guess,
                                      x_N = np.array([293.15,293.15,293.15]), # not used
                                      P0=P0,
                                      covar=ca.veccat(Q, R),
                                      codegen=True
                                      )

        sol_mhe.index = y_data.dt_index
        ax = sol_mhe["Ti"].plot(color="r")
        sol_mhe["y1"].plot(color="k", ax=ax)
        ax.legend(["model", "measured"])
        plt.show()
        params_mhe.to_csv("results/3R3C_vent_w_prior.csv", index=True)
        print(params_mhe)
    # dump for plots:
    sol_mhe.to_csv("results/linear_in_3R3C_w_prior.csv", index=True)