...
LimPIDJacobian::LimPIDJacobian(IGlobalSettings* globalSettings, shared_ptr<ISimObjects> simObjects)
    ...
    , _FMIDERjacobian(ublas::zero_matrix<double>(4,5))
    , _FMIDERjac_y(ublas::zero_vector<double>(4))
    , _FMIDERjac_tmp(ublas::zero_vector<double>(12))
...
void LimPIDJacobian::calcFMIDERJacobianColumn()
{
  double tmp14;
  double tmp15;
  _FMIDERjac_tmp(2) = ((_PI_P_addP_P_k1_ * _FMIDERjac_x(4)) + (_PI_P_addP_P_k2_ * _FMIDERjac_x(3)));
  ...
  if (_PI_P_gainPID_P_y_ > 1.0) {
    ...
    if (_PI_P_gainPID_P_y_ < 0.0) {
      ...
      tmp14 = _FMIDERjac_tmp(6);
    }
    tmp15 = tmp14;
  }
  _FMIDERjac_y(1) = tmp15;
  _FMIDERjac_tmp(4) = (_FMIDERjac_y(1) - _FMIDERjac_tmp(6));
  _FMIDERjac_tmp(7) = (_PI_P_gainTrack_P_k_ * _FMIDERjac_tmp(4));
  _FMIDERjac_tmp(0) = ((_PI_P_addI_P_k1_ * _FMIDERjac_x(4)) + ((_PI_P_addI_P_k2_ * _FMIDERjac_x(3)) + (_PI_P_addI_P_k3_ * _FMIDERjac_tmp(7))));
  _FMIDERjac_y(0) = (_PI_P_I_P_k_ * _FMIDERjac_tmp(0));
  _FMIDERjac_y(2) = (_gain_P_k_ * _FMIDERjac_x(1));
  _FMIDERjac_y(3) = (_gain1_P_k_ * _FMIDERjac_x(2));
}
...
matrix_t&  LimPIDJacobian::getFMIDERJacobian()
{
  ...
  calcFMIDERJacobianColumn();
  _FMIDERjac_x.clear();
  _FMIDERjacobian(2,1) = _FMIDERjac_y(2);
  _FMIDERjac_x(2) = 1;
  calcFMIDERJacobianColumn();
  _FMIDERjac_x.clear();
  _FMIDERjacobian(3,2) = _FMIDERjac_y(3);
  _FMIDERjac_x(3) = 1;
  calcFMIDERJacobianColumn();
  _FMIDERjac_x.clear();
  _FMIDERjacobian(0,3) = _FMIDERjac_y(0);
  _FMIDERjacobian(1,3) = _FMIDERjac_y(1);
  _FMIDERjac_x(4) = 1;
  calcFMIDERJacobianColumn();
  _FMIDERjac_x.clear();
  _FMIDERjacobian(0,4) = _FMIDERjac_y(0);
  _FMIDERjacobian(1,4) = _FMIDERjac_y(1);
