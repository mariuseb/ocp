...
params = pd.DataFrame(pd.read_csv("results/2R2C_vent_w_prior.csv", index_col=0))
# to get estimated x0, read trajectory:
sol = pd.read_csv("results/linear_in_2R2C_w_prior.csv", index_col=0)
...
y_data = Data.get_dataset(
                          ...
                          stop = pd.Timestamp("2024-01-19 00:00")
                          )
y_data = y_data.bfill()
...
result_gen = ResultGenerator(
                             config="configs/2R2C_int_gains.json",
                             params=params,
                             slack=False,
                             dt=300
                            )
...
result_gen.make_journal_plot(
                             y_data,
                             x0, 
                             "journal_plot_training_2R2C_vent"
                             )
plt.show()
res = result_gen.plot_residual_dist(
                                    y_data,
                                    "residual_plot_training_2R2C_vent"
                                    )
plt.show()
...
y_data = Data.get_dataset(
                          start = pd.Timestamp("2024-01-19 00:00"),
                          stop = pd.Timestamp("2024-01-21 00:00")
                          )
y_data = y_data.bfill()
y_data.index.name = "time"
y_data = y_data.groupby(pd.Grouper(freq='5min')).mean().dropna()
