...
params = pd.DataFrame(pd.read_csv("results/params_2R2C_vent_tvp_w_prior.csv", index_col=0))
# to get estimated x0, read trajectory:
sol = pd.read_csv("results/sol_2R2C_vent_tvp_w_prior.csv", index_col=0)
...
result_gen = ResultGenerator(
                             config="configs/2R2C_det_vent_tvp_vent_no_conns.json",
                             params=params,
                             dt=3600
                            )
...
p_base = pd.read_csv("results/tvp_params_base_2R2C_vent_jan24_w_prior.csv", index_col=0).values.flatten()
p_mod = pd.read_csv("results/tvp_params_mod_2R2C_vent_jan24_w_prior.csv", index_col=0).values.flatten()
...
result_gen.simple_one_step_plot(
                                ...
                                cond_series=y_data.ahu_reaFloSupAir,
                                switch=
                                    lambda x, p, p_mod: 
                                    p + p_mod
                                    if x > 10
