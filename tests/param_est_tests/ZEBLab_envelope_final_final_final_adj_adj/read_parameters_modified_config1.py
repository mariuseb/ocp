...
params_hist_jan = pd.read_csv("params_config1_dataset1.csv")
params_hist_nov = pd.read_csv("params_config1_dataset2.csv")
...
def set_first_and_scale(params_hist):
    ...
    params[["Ci_w", "Ce_w"]] /= 3.6E3*A
    params[["Rie", "Rea"]] *= A
    params[["Rie_w", "Rea_w"]] *= A
    params[["Ai", "Ai_high"]] /= A
...
for i in range(2):
    ...
    params[["Ci_w"]].plot(ax=axes[0,i], drawstyle="steps-post", color=cmap["Ci"], marker="*", linestyle="dashed")
    params[["Ce_w"]].plot(ax=axes[0,i], drawstyle="steps-post", color=cmap["Ce"], marker="*", linestyle="dashed")
    ...
    params[["Rie_w"]].plot(ax=axes[1,i], drawstyle="steps-post", color=cmap["Rie"], marker="*", linestyle="dashed")
    params[["Rea_w"]].plot(ax=axes[1,i], drawstyle="steps-post", color=cmap["Rea"], marker="*", linestyle="dashed")
