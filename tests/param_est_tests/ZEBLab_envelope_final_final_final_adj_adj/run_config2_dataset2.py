...
if __name__ == "__main__":
    ...
    prbs_res = pd.read_csv("prbs_val_jan.csv", index_col=0)
    prbs_res["Ti_sim"] = prbs_res["Ti_sim"].astype(float)
    
    result_gen.make_journal_plot_alt("whole_day_jan_2024", training=False, ref_result=prbs_res)  
      
    res = result_gen.val_res
    res["Ti_res"] = res["y1"] - res["Ti_onestep"]
    res["v1"] = res["y1"] - res["Ti_sim"]
    plot_residuals(res, res, "res_dist_config2_dataset2")
...
#    result_gen.make_journal_plot_alt("whole_day_jan_2024", training=False)
    result_gen.residuals.to_csv("to_CTSMR/residuals_config2_dataset2.csv")
    params_hist.to_csv("params_config2_dataset2.csv")
    train_metrics.to_csv("metrics/train_metrics_config2_dataset2.csv")
    val_metrics.to_csv("metrics/val_metrics_config2_dataset2.csv")
