...
if __name__ == "__main__":
    ...
    sampling_rate = "15min"
    _y_data, dt, _N = zeb_data.get_dataset(
                                        start=start, 
                                        stop=stop,
                                        sampling_rate=sampling_rate
                                        )
    actual_prbs = _y_data.phi_h
    ...
    # 3. Filter for non-zeros and count sequence lengths
    prbs_non_zero_lengths = actual_prbs[_mask].groupby(_block_id[_mask]).size()
    #print(_non_zero_lengths)
    
    prbs_non_zero = prbs_non_zero_lengths.copy()
    normal_non_zero = non_zero_lengths.copy()
    
    # go through and find closest:
    #closest_index = (df['values'] - target).abs().idxmin()
    #seqs = pd.DataFrame(columns=["length"])
    seqs = pd.Series([])
    for ndx in prbs_non_zero_lengths.index:
        target = prbs_non_zero_lengths.loc[ndx]
        closest_index = (non_zero_lengths - target).abs().idxmin()
        seqs.loc[closest_index] = non_zero_lengths.loc[closest_index]
        # remove that index:
        keep_inds = [
            ndx for ndx in non_zero_lengths.index
            if ndx != closest_index
        ]
        non_zero_lengths = non_zero_lengths.loc[keep_inds]
    print("tail")

    longest_n = seqs.iloc[0:3]
    shortest_m = seqs.iloc[3:]
