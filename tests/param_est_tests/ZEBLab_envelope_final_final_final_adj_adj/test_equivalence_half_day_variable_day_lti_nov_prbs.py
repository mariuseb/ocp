...
if __name__ == "__main__":
    ...
    # read prbs
    prbs_params = pd.read_csv("2R2C_prbs.csv", index_col=0)
    for k in prbs_params.index:
        param_guess[k]["lb"] = float(prbs_params.loc[k])
        param_guess[k]["ub"] = float(prbs_params.loc[k])
        param_guess[k]["init"] = float(prbs_params.loc[k])
    ...
    result_gen.residuals.to_csv("to_CTSMR/residuals_tvp_nov_covar_non_opt_nov_H=1_PRBS.csv")
    params_hist.to_csv("LTI_reiden_no_covar_opt_nov.csv")
    train_metrics.to_csv("metrics/train_metrics_LTI_no_reiden_covar_opt.csv")
    val_metrics.to_csv("metrics/val_metrics_LTI_no_reiden_covar_opt.csv")
