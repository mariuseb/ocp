...
#from pprint import pprint
#from utils import annotate_plot, faults, hashes
...
def set_constraints(df):
    ...
    df.loc[work_inds, "Ti_lb"] += 6
    df.loc[work_inds, "Ti_ub"] -= 4  
    return df
...
def plot_temp_power(
    ...
    power = "phi_h"
    index = df.index
    ax.plot(index.to_numpy(), df[temp].values, drawstyle="steps-post", color="k", linewidth=0.75, linestyle=linestyle, marker=marker, markevery=2)
    ax.plot(index.to_numpy(), df["y1"].values, drawstyle="steps-post", color="orange", linewidth=0.75, linestyle=linestyle, marker=marker, markevery=2)
    ax.plot(index.to_numpy(), df["Ti_lb"].values, drawstyle="steps-post", color="k", linestyle="dashed", linewidth=0.75)
    ax.plot(index.to_numpy(), df["Ti_ub"].values, drawstyle="steps-post", color="k", linestyle="dashed", linewidth=0.75)

    #faults_room = faults.get(room, {})
    #annotate_plot(df[temp], faults_room, ax)
        
    ax1 = ax.twinx()
    #(df[power]*1000).plot(ax=ax1, drawstyle="steps-post", color="r", linewidth=0.75, linestyle=linestyle, marker=marker, markevery=4)
    ax1.plot(index.to_numpy(), (df[power]).values, drawstyle="steps-post", color="r", linewidth=0.75, linestyle=linestyle, marker=marker, markevery=4)
    ax.set_title(title)
    #ax1.set_ylim([0, 3000])
    ax.set_ylim([15, 25])
    ax.set_yticks([15,20,25])
    #ax1.set_yticks([0,1000,2000])
    #ax.set_yticks([16,22,28])
    if set_labels:
        ax.set_ylabel("$\\mathrm{Indoor \\; temperature}$ $[\\mathrm{^\\circ C]}$", color="orange")
        #ax.yaxis.set_label_coords(x=-0.035, y=0.0)
        ax1.set_ylabel("$\\mathrm{Radiator \\; power \\; [W]}$", color="r", labelpad=0)
        #coords = ax1.yaxis.get_label_coords(x=1.05, y=0.2)
        #ax1.yaxis.set_label_coords(x=1.03, y=0.1)
    #ax.legend(["$T_i$",  "$"], ncol=2, loc=(0, 1.00))
    ax.legend(["$\\hat{T}_{\\mathrm{i},N|0}$"], ncol=1, loc="upper left")
    ax1.ticklabel_format(axis='y', style='sci', scilimits=(0,0))
    
def plot_weather(df, ax):
    #ax.plot(df.index,df.T_amb)
    #df["T_amb"].plot(ax=ax, drawstyle="steps-post", color="green", linewidth=0.75)
    index = df.index
    ax.plot(index.to_numpy(), df["Ta"].values, drawstyle="steps-post", color="green", linewidth=0.75)
    ax1 = ax.twinx()
    #ax1.plot(df.index, df.I_hor)
    ax1.plot(index.to_numpy(), df["phi_s"].values, drawstyle="steps-post", color="m", linewidth=0.75)
    ax1.ticklabel_format(axis='y', style='sci', scilimits=(0,0))
    ax.set_ylabel("$\\mathrm{Temperature}$ $[\\mathrm{^\\circ C]}$", color="green")
    #ax.yaxis.set_label_coords(x=-0.035, y=0.5)
    ax1.set_ylabel("$\\mathrm{Global \\; horizontal \\; radiation}$ $[\\mathrm{\\frac{W}{m^2}}]$", color="m")
    #ax1.yaxis.set_label_coords(x=1.03, y=0.6)

def plot_twin_rooms(
    df,
    faults_219: dict = dict(),
    faults_220: dict = dict()
    ):
    """
    Plot operation:
        - temperature vs. bounds
        - power
    """   
    #df.index = df.index + pd.Timedelta(days=1)
    df = set_constraints(df)
...
if __name__ == "__main__":
    ...
    fig, ax = plot_twin_rooms(sol)
    fig.savefig("PRBS_result.pdf")
    plt.show()
    ...
    residuals = pd.DataFrame([residual_non_opt, residual_opt]).T
