...
if __name__ == "__main__":
    ...
    cfg_path = os.path.join("configs", "3R3C_int_gains_sep_bal_vent.json")
    ...
    ekf_config = os.path.join("configs/ekf_configs", "3R3C_envelope_EKF_int_gains_sep_bal_vent.json") 
    #covar_kwargs = kwargs
    covar_kwargs = dict()
    ...
    """
    P0 = np.eye(3)*np.diag([0.026, 0.666,0.124])**2
    P0_guess = P0.flatten()
    Q_guess = np.array(
        ca.veccat(
    ...
    R_guess = np.array(ca.DM.eye(covar_solver.ekf.dae.n_y)).flatten()*1e-5
