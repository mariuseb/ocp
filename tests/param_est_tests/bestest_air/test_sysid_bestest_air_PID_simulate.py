...
if __name__ == "__main__":
    ...
    #cfg_path = os.path.join("configs", "3R2C_bestest_air_simple_LimPID.json")
    cfg_path = os.path.join("configs", "3R2C_bestest_air_simple_PID.json")
    ...
    PLOT_DATA = False
    sampling_freq = "30s"
    data_path = os.path.join("data", "data_bestest_air_simple_normal_op_Simple_PID.csv")
    repeats = 1
    ...
    for n in range(N+1):
        ...
        ys = np.append(ys, np.array(y_PI))
        
    res = pd.DataFrame(data=xs.reshape(N+1, 3), columns=["Ti", "Te", "E"])
    #res = pd.DataFrame(data=xs.reshape(N+1, 3), columns=["Ti", "Te"])
