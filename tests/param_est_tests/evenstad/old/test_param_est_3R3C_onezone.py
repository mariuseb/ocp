...
if __name__ == "__main__":
    ...
    y_data = prepare_dataset(data4, plot=False, regenerate_weather=False) 
    ...
    with ParameterEstimation(config=cfg_path,
        ...
        test = prepare_dataset(data2) #, columns=columns)
        test.index = pd.to_datetime(test.index)
        #test = prepare_dataset(data5, columns=columns)
        # resample to 1H:
        phi_h = test.phi_h.resample(rule="1H").mean() #.shift(2)
        test = test.resample(rule="1H").asfreq()
        test["phi_h"] = phi_h
        
        test = test[1:]
        test.index = y_data.index
        F = param_est.integrator.one_sample.mapaccum(len(test))
        #x0 = sol[["Ti", "Te", "Th"]].iloc[0].values
        x0 = ca.DM([test.Ti[0], test.Ti[0] -  2, test.Ti[0] + 2])
        ...
        res = F(x0=x0, z=0, u=u_data, p=p_data, s=0, r=0)
        Ti_res = res["xf"][0, :]
        #Ti1_res = res["xf"][2, :]
        test_res = pd.DataFrame(columns=["y1", "Ti_res"])
        test_res["y1"] = test.y1
