...
if __name__ == "__main__":
    ...
    # exclude also monday:
    #weekdays = [ndx for ndx in y_data.index if ndx.dayofweek not in (4,5,6,0,1)]
    #weekends = [ndx for ndx in y_data.index if ndx.dayofweek in (4,5,6,0,1)]
    # start weekend at 
    ...
    dt_index = pd.Series(y_data.index)
    weekends = dt_index.apply(filter_weekend)
    weekends = pd.DatetimeIndex(dt_index.loc[weekends])
    
    weekdays = [ndx for ndx in y_data.index if ndx not in weekends]
    
    #weekend = [ndx for ndx in y_data.index if ndx.dayofweek not in (5,6,0)]
    feb_weekdays = [ndx for ndx in weekdays if ndx.month == 2]
    feb_weekends = [ndx for ndx in weekends if ndx.month == 2]
    jan_feb_weekdays = [ndx for ndx in weekdays if ndx.month in (1,2)]
    jan_weekdays = [ndx for ndx in weekdays if ndx.month in (1,)]
    jan_weekends = [ndx for ndx in weekends if ndx.month in (1,)]
    feb = [ndx for ndx in y_data.index if ndx.month == 2]
    feb_mar = [ndx for ndx in y_data.index if ndx.month in (2,3)]
    #y_data = y_data.loc[feb_mar]
    y_data = y_data.loc[feb_weekdays]
    #y_data = y_data.loc[feb_weekends]
    #y_data = y_data.loc[feb]
    ax = y_data["Ti"].plot()
    plt.show()
