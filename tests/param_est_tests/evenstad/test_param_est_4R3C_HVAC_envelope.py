...
if __name__ == "__main__":
    ...
    cfg_path = "configs/HVAC_only_rad_vent.json"
    ...
    df.V_flow_exh_air[df.V_flow_exh_air < 20000] = 0
    ...
    phi_h = df["P_water"].resample(rule=sampling_time).mean()
    y_data = df.resample(rule=sampling_time).asfreq()
    #phi_h = df["P_water"]
    #y_data = df
    y_data["phi_h"] = phi_h # to Watts
    y_data["P_water"] = phi_h # resampled
    y_data["Tsup_rad"] = y_data["Tsup_water_west"]
    y_data["Tret_rad"] = y_data["Tret_water_west"]
    y_data["Tsup_air"] = y_data["Tsup_air"]
    y_data["Tret_air"] = y_data["Tret_air"]
    y_data["m_flow_water"] = (y_data["Tsup_rad"] - y_data["Tret_rad"])*4200/(y_data["phi_h"])
    y_data["y1"] = y_data["Ti"]
    y_data["y2"] = y_data["P_water"]
    ...
    if PLOT:
        ...
        plt.show()
        #ax1 = ax.twinx()
        #y_data.valve.plot(ax=ax1, color="k")
        #plt.show()
    ...
    with ParameterEstimation(config=cfg_path,
        ...
        (sol["P_rad"] + sol["P_vent"]).plot(color="r", ax=ax, drawstyle="steps-post")
        sol["y2"].plot(color="k", ax=ax, drawstyle="steps-post")
        ax.legend()
        plt.show()
        print(params)
        print(param_est.get_taus(params.values))
