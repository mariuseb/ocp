...
if __name__ == "__main__":
    ...
    cfg_path = os.path.join("configs", "4R2C_vent_no_waterside_rad.json")
    ...
    data_path = os.path.join(get_data_path(), "data_singlezone_commercial_combined_rad_vent.csv")
    ...
    if PLOT:
        ...
        ax1 = ax[1].twinx()
        y_data.ahu_reaTSupAir.plot(ax=ax1, color="k", drawstyle="steps-post")
        y_data.Pvent.plot(ax=ax[2], drawstyle="steps-post")
        y_data.Prad.plot(ax=ax[2], drawstyle="steps-post")
        ax2 = ax[2].twinx()
        y_data.Ti.plot(ax=ax2, color="k", drawstyle="steps-post")
        plt.show()
    ...
    kwargs = {
        ...
        #"p_nom": [1E-5,1E-4,1E8,1E9,1E2,1,1E3,1,12],
        #"p_nom_b": [0,0,0,0,0,0,0,0,289.15],
        "y_nom": [12,1E6,12,1,1,12,1E6],
        "y_nom_b": [289.15,0,289.15,0,0,289.15,0]
    }
    kwargs = {
        "x_nom": 1,
        "u_nom": 1,
    ...
    with ParameterEstimation(config=cfg_path,
        ...
        #v8 = param_est.nlp["x"][(v_inds["a"]+7):v_inds["b"]:param_est.dae.n_y]
        #v6 = param_est.nlp["x"][(v_inds["a"]+5):v_inds["b"]:param_est.dae.n_y]
    ...
    Ph_rad = (y_data.dh_pump*params["UA_nom"]*(params["dh_Tsup"] - y_data["Ti"])**params["n3"])/params["eta_hex3"]
    ax = (y_data.Prad*params["eta_hex1"]).plot(drawstyle="steps-post")
    Ph_rad.plot(ax=ax, drawstyle="steps-post", linestyle="dashed")
    #ax.legend(["Pwater", "Pvent", "Pair"])
    ax.legend()
    plt.show()
