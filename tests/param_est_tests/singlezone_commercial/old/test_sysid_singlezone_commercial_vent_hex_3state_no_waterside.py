...
if __name__ == "__main__":
    ...
    cfg_path = os.path.join("configs", "4R3C_no_waterside.json")
    ...
    y_data = data[1:-2]
    y_data["y1"] = y_data["Ti"]
    y_data["y2"] = y_data["ahu_reaTSupAir"]
    #y_data["y2"] = y_data["Ph"]
    ...
    if PLOT:
        ...
        # water-side power
        y_data["Ph_water"] = y_data["oveValCoi"]*2*1000*30*(y_data["ahu_reaTCoiSup"] - y_data["ahu_reaTCoiRet"])*0.85
    ...
    with ParameterEstimation(config=cfg_path,
        ...
        plt.show()
        # AHU temp plot:
        ax = sol["ahu_reaTSupAir"].plot(color="r")
        sol["y2"].plot(color="k", ax=ax)
        ax1 = ax.twinx()
        sol["Pvent"].plot(ax=ax1, drawstyle="steps-post")
        ax.set_ylim([290,315])
        ax.legend()
        plt.show()
        print(params)
