...
if __name__ == "__main__":
    ...
    data["rad_flo_on"] = (data.rad_flo > 1E-4).astype(int) + 1E-4
    ...
    cfg_path = "configs/HVAC_model_DAE_full_Tret_adj_simpler.json"
    ...
    #x_guess = y_data[["Tret","Tsup_air","rad_flo"]].values.T        
    x_guess = y_data[["Tret","Tsup_air"]].values.T        
    N = len(y_data) 
    ...
    with ParameterEstimation(config=cfg_path,
        ...
        y = param_est.dae.y
        fig, _axes = plt.subplots(8,2, sharex=True)
        axes = _axes.flatten()
    ...
    params.to_csv("HVAC_DAE_model_latest.csv", index=True)   