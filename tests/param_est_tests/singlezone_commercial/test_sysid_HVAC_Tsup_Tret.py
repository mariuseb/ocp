...
if __name__ == "__main__":
    ...
    """
    Diff before resampling:
    """
    data["Qrad_Ws"] = data.Qrad/(1/3.6E6) # from kWh to Ws
    #data["Prad"] = data["Prad"].shift(-1) 
    #data["Prad_alt"] = data.Qrad.diff(1)*6E4s
    data["Prad_alt"] = ((data.Qrad_Ws).diff(1))/60
    
    #data = data.resample(rule=sampling_rate).asfreq()
    data = data.resample(rule=sampling_rate).mean()
    # special handling of these:_
    #integrate = ["rad_flo", "coi_flo", "Pvent", "Prad", "Tsup", "Tret", "Ti"]
    #integrate = ["Pvent", "Prad", "Ti"]
    #for name in integrate:
    #    data[name] = old_data[name].resample(rule=sampling_rate).mean()
    ...
    data["rad_flo_on"] = (data.rad_flo > 1E-4).astype(int)
    ...
    ax.legend()
    ax = axes[2]
    data[["rad_val", "rad_flo"]].plot(ax=ax, linewidth=0.75, drawstyle="steps-post")
    ax.legend()
    ...
    cfg_path = "configs/HVAC_model_DAE_Tret_Tsup_alt_alt.json"
    ...
    with ParameterEstimation(config=cfg_path,
        ...
        for i, (meas, var) in enumerate(y.items()):
            ax = axes[i]
            sol[meas].plot(ax=ax, color="k", linestyle="dashed", drawstyle="steps-post", linewidth=0.75)
            sol[var.name()].plot(ax=ax, color="r", linewidth=0.75, drawstyle="steps-post")
            ax.legend()
        ...
        ax = sol["Tsup"].plot(color="r", linestyle="dashed", linewidth=0.75)
        sol["Tret"].plot(ax=ax, color="y", linewidth=0.75)
        sol["Trad"].plot(ax=ax, color="g", linewidth=0.75)
    ...
    params.to_csv("HVAC_DAE_model_latest_Tsup_Tret_5min.csv", index=True)   
