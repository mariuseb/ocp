...
if __name__ == "__main__":
    ...
    cfg_path = "configs/6R5C.json"

    kwargs = {
        "x_nom": 12,
        "x_nom_b": 289.15,
        "u_nom": [1E6,1E6,1,1,1,1,12,12],
        "u_nom_b": [0,0,0,0,0,0,289.15,289.15],
    ...
    for k in params_hvac.index:
        val = float(params_hvac.loc[k])
        param_guess[k] = {
            "init": val,
            "lb": val,
            "ub": val
    ...
    x_guess = y_data[["Ti", "Ti", "Tret","Tret","Tsup"]].values.T
    ...
    params.to_csv("envelope_model_latest_6R5C.csv", index=True)  