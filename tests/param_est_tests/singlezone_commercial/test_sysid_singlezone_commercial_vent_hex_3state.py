...
if __name__ == "__main__":
    ...
    cfg_path = os.path.join("configs", "4R3C_inf_vent.json")
    ...
    if PLOT:
        ...
        # water-side power
        y_data["Ph_water"] = y_data["oveValCoi"]*2*1000*(y_data["ahu_reaTCoiSup"] - y_data["ahu_reaTCoiRet"])*0.85
        y_data["Ph_air"] = y_data.ahu_reaFloSupAir*32*1*(y_data["ahu_reaTSupAir"] - y_data["ahu_reaTHeaRec"])
    ...
    kwargs = {
        "x_nom": 12,
        "x_nom_b": 289.15,
        "u_nom": [1E6,1,1,12,12,12],
        "u_nom_b": [0,0,0,289.15,289.15,289.15],
        "r_nom": [12,300,1E6,1E6,1E6],
        "r_nom_b": [289.15,0,0,0],
        "p_nom": [1E-5,1E-4,1E8,1E9,1E2,1,1E3,1,12],
        "p_nom_b": [0,0,0,0,0,0,0,0,289.15],
        "y_nom": [12,12],
    ...
    x_guess = np.array([y_data.Ti.values.flatten(), y_data.Ti.values.flatten(), y_data.ahu_reaTSupAir.values.flatten()])
    ...
    with ParameterEstimation(config=cfg_path,
        ...
        eta_hex = param_est.dae.dae.var("eta_hex")
        #v2 = v2/eta_rad
        v2 = v2/eta_hex
        param_est.nlp["f"] = \
            R[0,0]*ca.dot(v1, v1) + \
