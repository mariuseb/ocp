...
if __name__ == "__main__":
    ...
    y_data = data.resample(rule="5min").mean()
    ...
    cfg_path = "configs/4R4C_mod.json"

    kwargs = {
        "x_nom": 12,
        "x_nom_b": 289.15,
        "u_nom": [1E6,1E6,1,1,1,12,1,12,12,1],
        "u_nom_b": [0,0,0,0,0,289.15,0,289.15,289.15,0],
    ...
    for name in to_set:
        val = float(params_hvac.loc[name])
        param_guess[name] = {
            "init": val,
            "lb": val,
            "ub": val,
    ...
    params.to_csv("envelope_model_latest_4R4C.csv", index=True)  