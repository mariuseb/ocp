...
if __name__ == "__main__":
    ...
    data_path = os.path.join(
                            get_data_path(), 
                            "data_bestest_hydronic_normal_test_rad_val_1.csv"
                            )    
    N = 24
    ...
    if GENERATE_DATA_NORMAL_OP:
        ...
        for n in range(N):
            ...
            #u_0 = pd.Series(index=['rad_val'], data=[1])
            u_0["oveValCoi"] = 1
            u_0["rad_val"] = 0
    ...
    cfg_path = "configs/n_power_mass_flow_TRec_Tret_Prad_NN.json"
    ...
    """
    valve_model = NN(1,1)
    valve_model = torch.load("val_flo_map.pth")
    
    """
    Test torch model first:
    """
    x_nn = data[["rad_val"]] #[:-M+1]
    #x_scale = (x_nn.max()-x_nn.min())
    y_nn = data[["rad_flo"]] #[:-M+1]
    ...
    model_y = valve_model(x/NN_x_scale)
    ...
    x = y_data.rad_val.values
    y = np.array([])
    for n in range(N):
        y = np.append(y, f(x[n]))
    y_data["test_NN"] = y
    y_data[["rad_flo", "test_NN"]].plot()
    plt.show()
