...
if __name__ == "__main__":
    ...
    data_path = os.path.join(
                            get_data_path(), 
                            "data_bestest_hydronic_PRBS_op_60s.csv"
                            )    
    ...
    ax = data.Trad.plot(color="k", linestyle="dashed", drawstyle="steps-post", linewidth=0.75)
    ax1 = ax.twinx()
    data.rad_flo.plot(color="r", drawstyle="steps-post", linewidth=0.75)
    plt.show(block=False)

    ax = data.Ti.plot(color="k", linestyle="dashed", drawstyle="steps-post", linewidth=0.75)
    ax1 = ax.twinx()
    data.Prad.plot(color="r", drawstyle="steps-post", linewidth=0.75)
    plt.show(block=False)
    ...
    y_data = data.resample(rule=sampling_time).mean()
    integrate = ["rad_flo", "coi_flo", "Pvent", "Prad"]
    for name in integrate:
        y_data[name] = data[name].resample(rule=sampling_time).mean()
    y_data["rad_flo"] = y_data["rad_flo"].shift(-1) 
    y_data = y_data.iloc[:-1]

    ax = y_data.Trad.plot(color="k", linestyle="dashed", drawstyle="steps-post", linewidth=0.75)
    ax1 = ax.twinx()
    #y_data.Prad.plot(ax=ax1, color="r", drawstyle="steps-post", linewidth=0.75)
    y_data.rad_flo.plot(ax=ax1, color="r", drawstyle="steps-post", linewidth=0.75)
    plt.show(block=False)
    
    ax = y_data.Ti.plot(color="k", linestyle="dashed", drawstyle="steps-post", linewidth=0.75)
    ax1 = ax.twinx()
    #y_data.Prad.plot(ax=ax1, color="r", drawstyle="steps-post", linewidth=0.75)
    y_data.Prad_to_env.plot(ax=ax1, color="r", drawstyle="steps-post", linewidth=0.75)
    plt.show(block=False)
    ...
    ax = y_data.Ti.plot(color="k", linestyle="dashed", drawstyle="steps-post", linewidth=0.75)
    ax1 = ax.twinx()
    y_data.Prad.plot(color="r", drawstyle="steps-post", linewidth=0.75)
    y_data.Pvent.plot(color="y", linewidth=0.75)
    plt.show(block=False)
    """

    #cfg_path = "configs/4R3C_Tsup_input_energy_cons.json"
    cfg_path = "configs/4R3C_stoch.json"

    kwargs = {
        "x_nom": 12,
        "x_nom_b": 289.15,
        "u_nom": [1E6,1E6,1,1,1,12,1,12,12,12],
        "u_nom_b": [0,0,0,0,0,289.15,0,289.15,289.15,289.15],
    ...
    params_hvac = pd.read_csv("HVAC_DAE_model_latest_Trad_Tret_mod_1min.csv", index_col=0)
    ...
    with ParameterEstimation(config=cfg_path,
        ...
        fisher_p = param_est.get_covar_p_unscaled(params.values, R, Q)
        ...
        ax = sol.Trad.plot(color="k", linestyle="dashed", drawstyle="steps-post", linewidth=0.75)
        sol.Ti.plot(ax=ax, color="g", linestyle="dashed", drawstyle="steps-post", linewidth=0.75)
        ax1 = ax.twinx()
        sol.rad_flo.plot(color="r", drawstyle="steps-post", linewidth=0.75)
        plt.show(block=False)
     
    
        
    #params.to_csv("envelope_model_latest_4R3C_mod_15min.csv", index=True)  
    #params.to_csv("envelope_model_latest_4R3C_rad_flo_shifted_5min.csv", index=True)  
    #params.to_csv("envelope_model_latest_4R3C_rad_flo_non_shifted_15min.csv", index=True)  
    #params.to_csv("envelope_model_latest_4R3C_rad_flo_shifted_15min.csv", index=True)  
    params.to_csv("envelope_model_latest_4R3C_rad_free_params.csv", index=True)  
    fisher_p.to_csv("envelope_model_latest_4R3C_rad_free_fisher.csv", index=True)  