...
if __name__ == "__main__":
    ...
    #cfg_path = os.path.join("configs", "4R2C_inf_Tsup_int_gains_true_Prad.json")
    ...
    y_data = data[:-2]
    y_data["pump_on"] = y_data["dh_pump"].astype(bool).astype(int)
    
    y_data["y1"] = y_data["Ti"]
    y_data["y2"] = y_data["Prad"]
    y_data["y3"] = y_data["rad_flo"]
    y_data["y4"] = y_data["Tsup"]
    y_data["y5"] = y_data["Tret"]
    #y_data["y2"] = y_data["Ph"]
    ...
    if PLOT:
        ...
        # plot energy vs. Tret:
        pump = data.dh_pump.sort_values(ascending=False)
        rad_flo = data.loc[pump.index].rad_flo
        Tret = data.loc[pump.index].Tret
        Tsup = data.loc[pump.index].Tsup
        pump.index = range(len(pump.index))
        Tret.index = range(len(pump.index))
        Tsup.index = range(len(pump.index))
        rad_flo.index = range(len(rad_flo.index))
        
        ax = valve.plot(drawstyle="steps-post", color="r")
        (rad_flo/rad_flo.max()).plot(ax=ax, drawstyle="steps-post", color="k")
        ax1 = ax.twinx()
        Tret.plot(ax=ax1, drawstyle="steps-post")
        Tsup.plot(ax=ax1, drawstyle="steps-post")
        plt.show()
    ...
    param_est_ode = ParameterEstimation(config="configs/4R2C_env_ODE.json",
                                        functions=functions,
                                        N=N,
                                        dt=dt,
    ...
    v_inds = param_est_ode.nlp_parser["v"]["range"]
    v1 = param_est_ode.nlp["x"][v_inds["a"]:v_inds["b"]]
    #param_est_ode.nlp["f"] = R[0,0]*0.5*ca.dot(v1, v1)
    sol, params = param_est_ode.solve(
                                    y_data,
                                    param_guess,
                                    lbp=lbp,
                                    ubp=ubp,
    ...
    param_est_ae = ParameterEstimation(config="configs/4R2C_HVAC_AE.json",
                                        functions=functions,
                                        N=N,
                                        dt=dt,
                                        param_guess=param_guess,
    ...
    v2 = param_est_ae.nlp["x"][(v_inds["a"]):v_inds["b"]:param_est_ae.dae.n_y]
    v3 = param_est_ae.nlp["x"][(v_inds["a"]+1):v_inds["b"]:param_est_ae.dae.n_y]
    v4 = param_est_ae.nlp["x"][(v_inds["a"]+2):v_inds["b"]:param_est_ae.dae.n_y]
    v5 = param_est_ae.nlp["x"][(v_inds["a"]+3):v_inds["b"]:param_est_ae.dae.n_y]
    # what to do with this? 
    eta = param_est_ae.dae.dae.var("eta")
    v2 = v2/eta
    Q = ca.DM.eye(0)
    R = ca.DM.eye(4)    
    R[0,0] = 1E-9
    ...
    sol_ae, params_ae = param_est_ae.solve(
                                    y_data,
                                    param_guess,
                                    lbp=lbp,
                                    ubp=ubp,
    ...
    x_guess = np.array(
                        [
                         sol.Ti.values.flatten(),
                         sol.Te.values.flatten()#,
                         #y_data.Tsup.values.flatten()
                         ]
                      )
                    #, y_data.ahu_reaTSupAir.values.flatten()])
    x_guess = x_guess.T.flatten()
    # update estimate of Prad:
    y_data["y1"] = y_data["Ti"]
    y_data["Prad"] = y_data["y2"] = sol_ae["v2"] + sol["y2"]*params_ae["eta"]
    Q = ca.DM.eye(2)
    R = ca.DM.eye(5)
    sol_2nd, params_2nd = param_est_ode.solve(
                                        y_data,
                                        params,
                                        lbp=lbp,
                                        ubp=ubp,
    ...
    ax = sol_2nd["Ti"].plot(color="r")
    sol_2nd["y1"].plot(color="k", ax=ax)
    ax.set_ylim([290,315])
    ax.legend()
    plt.show()
    
    # plot Tret model vs Tret actual:
    pump = y_data.dh_pump.sort_values(ascending=False)
    rad_flo = y_data.loc[pump.index].rad_flo
    Tsup_model = sol_ae.loc[pump.index].Tsup_model
    Tsup = y_data.loc[pump.index].Tsup
    pump.index = range(len(pump.index))
    Tsup_model.index = range(len(pump.index))
    Tsup.index = range(len(pump.index))
    rad_flo.index = range(len(rad_flo.index))
    ...
    ax = pump.plot(drawstyle="steps-post", color="r")
    (rad_flo/rad_flo.max()).plot(ax=ax, drawstyle="steps-post", color="k")
    ax1 = ax.twinx()
    Tsup_model.plot(ax=ax1, drawstyle="steps-post")
    Tsup.plot(ax=ax1, drawstyle="steps-post")
    ax1.legend()
    plt.show()
    ...
    pump = y_data.dh_pump.sort_values(ascending=False)
    rad_flo = y_data.loc[pump.index].rad_flo
    Tret_model = sol_ae.loc[pump.index].Tret_model
    Tret = y_data.loc[pump.index].Tret
    pump.index = range(len(pump.index))
    Tret_model.index = range(len(pump.index))
    Tret.index = range(len(pump.index))
    rad_flo.index = range(len(rad_flo.index))
    
    # set Tret_model equal to Tret when Tret_model == 0:
    to_equal_ndx = pump[pump==0].index
    Tret_model[to_equal_ndx] = Tret[to_equal_ndx]
    
    ax = pump.plot(drawstyle="steps-post", color="r")
    (rad_flo/rad_flo.max()).plot(ax=ax, drawstyle="steps-post", color="k")
    ax1 = ax.twinx()
    Tret_model.plot(ax=ax1, drawstyle="steps-post")
    Tret.plot(ax=ax1, drawstyle="steps-post")
    ax1.legend()
    plt.show()
        
    # plot Prad_model vs. Prad:
        
        
    # compare inferred heat with measured:  
    Prad_model = sol_ae["v2"] + sol["y2"]*params_ae["eta"]
    ax = Prad_model.plot(color="k", drawstyle="steps-post", linestyle="dashed")
    #ax1 = ax.twinx()
    #(y_data.Prad*params_ae["eta"]).plot(color="r", drawstyle="steps-post", linestyle="dashed", ax=ax)
    (y_data.Prad).plot(color="r", drawstyle="steps-post", linestyle="dashed", ax=ax)
    #ax.legend(["deltaT"])
