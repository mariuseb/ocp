...
if __name__ == "__main__":
    ...
    params = pd.read_csv("parameters_LTV_nov_2023.csv", index_col=0).squeeze()
    sol = pd.read_csv("solution_LTV_nov_2023.csv", index_col=0).squeeze()
    ...
    Q_disc = np.array([[np.sqrt(np.exp(Q[0,0])), 0],
                       [0, np.sqrt(np.exp(Q[1,1]))]])
