...
if __name__ == "__main__":
    ...
    cfg_path = os.path.join(opt_config_base, "3R2C_bestest_air_ufan_1meas.json")
    ...
    GENERATE_DATA = False
    PLOT_DATA = False
    data_path = os.path.join(get_data_path(), "data_bestest_air_simple.csv")
    ...
    if GENERATE_DATA:
        ...
        _u = pd.DataFrame(data=U.values, index=U.index, columns=["Tsup"])
        _u *= 25
        _u += 288.15 
        _u["u_fan"] = U
        # of time steps:
