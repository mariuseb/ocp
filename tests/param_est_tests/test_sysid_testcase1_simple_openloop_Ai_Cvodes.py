...
if __name__ == "__main__":
    ...
    cfg_path = os.path.join(opt_config_base, "2R2C_testcase1_cvodes.json")
    ...
    if GENERATE_DATA:
        ...
        y_data["y1"] = y_data.Ti
        y_data["phi_h"] = y_data["phi_h"].shift(-2).ffill().bfill()
        y_data["Tset"] = y_data["Tset"].shift(-2).ffill().bfill()
    ...
    integrator from it.
    """
    #dae = ca.DaeBuilder('Simple2R2C', 'Simple2R2C_inputs/Simple2R2C_inputs.fmutmp')
    ...
    dae.disp(True)
    #dae.set('p', [0.001, 0.01, 3.6E6*1, 3.6E6*5, 5])
    p_names = dae.p()
    x_names = dae.x()
    params = {"Ai.y": 5,
              "Ci.C": 1E6,
    ...
    for k,v in params.items():
        dae.set(k, v)
    ode = dae.create('f', ['x','u'], ['ode'], {"verbose": True})
    #ode = dae.create('f', ['t', 'x', 'z', 'p', 'u'], ['ode','alg','quad'], {"verbose": True})
    tgrid = [0,dt]
    #_N = 1
    ...
    one_sample_FMU = ca.Function('one_sample',[states,controls], [X])
    ...
    for n in range(N):
        ...
        #x0 = F(x0,0,0,ca.vertcat(u,r),0,0,0)[0]
