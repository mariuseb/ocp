...
if __name__ == "__main__":
    ...
    sysid using PRBS.
    """
    cfg_path = os.path.join("configs", "5R5C.json")
    ...
    for param in params_hvac.index:
        param_guess[param] = {
            ...
            "lb": params_hvac.loc[param].values[0]*1e-2,
            "ub": params_hvac.loc[param].values[0]*1E2
        }
    for param in ("Tsup_offset", "Tset_sup_b", "Tset_sup_a", "Ta_dim", "cp_water"):
        param_guess[param] = {
            "init": params_hvac.loc[param].values[0],
    ...
    with Estimation(
        ...
        # P_to_rad:
        sol["P_to_rad"] = (sol["Trad"] - sol["Ti"])/sol["Rirad"]
        
        sol["Prad"].plot(drawstyle="steps-post",ax=ax1)
        sol["P_to_rad"].plot(drawstyle="steps-post",ax=ax1, color="g")
        plt.show()
        params.to_csv("envelope_model.csv", index=True)
        print(params) 
