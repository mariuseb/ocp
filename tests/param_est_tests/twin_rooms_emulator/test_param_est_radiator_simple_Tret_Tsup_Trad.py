...
if __name__ == "__main__":
    ...
    cfg_path = os.path.join(
        "configs",
        "radiator_model_simple_Tret_Tsup_Trad.json"
    )
    ...
    y_data.index *= dt
    y_data["m_flow_bool"] = ((y_data["rad_flo"].round(5) > 0) | y_data["rad_val"].round(5) > 0).astype(int) + 1e-2
    ...
    kwargs = {
        ...
        "z_nom": [1e-1,1E3,12,1E3,1E3,1E3],
        "z_nom_b ": [0,0,289.15,0,0,0],
    ...
    params_env = pd.read_csv("envelope_model.csv", index_col=0)
    for param in ("Crad", "Rirad"):
        param_guess[param] = {
            "init": params_env.loc[param].values[0],
            "lb": params_env.loc[param].values[0],
            "ub": params_env.loc[param].values[0]
        }
    ...
    with Estimation(
        ...
        sol["P_to_rad"].plot(color="g", ax=ax, linewidth=0.75, drawstyle="steps-post")
        sol["P_to_env"].plot(color="m", ax=ax, linewidth=0.75, drawstyle="steps-post")
        ...
        sol["Trad"].plot(color="g", ax=ax, linewidth=0.75, drawstyle="steps-post")
        (y_data["Ta"] + 40).plot(color="r", ax=ax, linewidth=0.75, drawstyle="steps-post")
        ...
        params.to_csv("hvac_model.csv", index=True)
        print(params) 
