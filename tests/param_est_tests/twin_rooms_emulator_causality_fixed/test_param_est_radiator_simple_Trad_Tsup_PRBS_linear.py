...
if __name__ == "__main__":
    ...
    cfg_path = os.path.join(
        "configs",
        "radiator_model_simple_Trad_Tsup_linear.json"
    )
    ...
    env_data = pd.read_csv("solution_envelope_3R2C.csv", index_col=0)
    env_data.index = pd.to_timedelta(env_data.index)
    y_data["Ti"] = env_data["Ti"]
    #y_data["rad_val"] = y_data["rad_val"].shift(-1)
    ...
    with Estimation(
        ...
        sol["dT_model"] = sol["Tsup"] - sol["Trad"]
        ...
        ax.legend(["model", "measured"])
        ax = axes[2]
        sol["Trad"].plot(color="r", ax=ax, linewidth=0.75, drawstyle="steps-post")
        ...
        ax1 = ax.twinx()
        sol["m_flow_bool"].plot(color="b", ax=ax1, linewidth=0.75, drawstyle="steps-post")
