...
if __name__ == "__main__":
    ...
    y_data, N, dt = prepare_data(data_path, rule="10min")
    param_guess, kwargs, lbx, ubx, x_guess = prepare_est(y_data,)
    params = pd.read_csv("full_model_4R4C_1meas.csv", index_col=0)
    p_nom = pd.read_csv("p_nom_4R4C_1meas.csv", index_col=0)
    sol = pd.read_csv("simulation_traj_4R4C_1meas.csv", index_col=0)
    ...
    ekf_config = pathlib.Path(
        os.path.join("ekf_configs", "4R3C_1meas_EKF_adj.json") 
    )       
