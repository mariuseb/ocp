...
if __name__ == "__main__":
    ...
    sysid using PRBS.
    """
    cfg_path = os.path.join("configs", "2R3C_alt_no_dT.json")
    data_path = os.path.join(
                            "twin_rooms_emulator_multiple_power_levels_15min.csv"
                            )
    ...
    y_data = y_data.iloc[180:]
    N = len(y_data)
    y_data["Prad"] = y_data["Prad_calc"]
    y_data["rad_flo"] = y_data["rad_flo_calc"]
    param_guess, kwargs, lbx, ubx, x_guess = prepare_est(y_data, n_x=3, hvac=True)
    
    kwargs["z_nom"] = [1E4]
    kwargs["z_nom_b"] = [0]
    kwargs["y_nom"] = [12,1e-1,1E4,12]
    kwargs["y_nom_b"] = [289.15,0,0,0]
    ...
    with Estimation(
        ...
        sol["y3"].plot(color="g", ax=ax, linewidth=0.75, drawstyle="steps-post")
        ax.legend(["model", "measured"])
        plt.show()

        ax = sol["rad_flo"].plot(color="r", linewidth=0.75, drawstyle="steps-post")
        sol["y2"].plot(color="g", ax=ax, linewidth=0.75, drawstyle="steps-post")
        sol["rad_219"].plot(color="k", ax=ax, linewidth=0.75, drawstyle="steps-post")
        ax.legend(["model", "measured"])
        plt.show()
        
        ax = sol["dT"].plot(color="r", linewidth=0.75, drawstyle="steps-post")
        sol["y6"].plot(color="g", ax=ax, linewidth=0.75, drawstyle="steps-post")
        ax.legend(["model", "measured"])
        plt.show()
