...
if __name__ == "__main__":
    ...
    sysid using PRBS.
    """
    cfg_path = os.path.join("configs", "2R3C_alt_alt_alt.json")
    ...
    param_guess, kwargs, lbx, ubx, x_guess = prepare_est(y_data, n_x=4)
    ...
    params = pd.read_csv(
        "envelope_model_2R3C_alt_alt.csv",
        index_col=0
    )
    
    for param in ["Rie", "Rea", "Ci", "Ce", "Ai"]:
        param_guess[param] = {
            "init": params.loc[param].values[0],
            "lb": params.loc[param].values[0],
            "ub": params.loc[param].values[0]
        }
