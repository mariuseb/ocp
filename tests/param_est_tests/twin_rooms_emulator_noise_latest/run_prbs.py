...
if __name__ == "__main__":
    ...
    if GENERATE_DATA:
        ...
        # run prbs:
        for n in range(N):
            u_0 = pd.Series(index=['rad_219'], data=[prbs.iloc[n]])
            #u_0 = pd.Series(index=['rad_219'], data=[0.0])
            #u_0["rad_220"] = 0.0
            #u_0 = pd.Series(index=['rad_219'], data=[0])
            #u_0["rad_220"] = prbs.iloc[n]
            #u_0["fan_220"] = 1e-1
            #u_0["fan_219"] = 1e-1
            #u_0["fan_ret_220"] = 1e-1
            #u_0["fan_ret_219"] = 1e-1
            #u_0["fan_220"] = 0
            #u_0["fan_219"] = 0
            try:
        ...
        #data = boptest.get_results(tf=N*boptest.h + 672*boptest.h)
        # check decay:
        for n in range(2*96):
            #u_0 = pd.Series(index=['rad_219'], data=[0.0])                  
            #_, y_meas, u_meas = boptest.evolve(u=u_0)
