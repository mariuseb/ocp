...
if __name__ == "__main__":
    ...
    data_path = os.path.join(
                            "twin_rooms_emulator_PRBS_mod_15min.csv"
                            )
    y_data, N, dt = prepare_data(data_path, rule="15min")
    ...
    with Estimation(
        ...
        ax1 = ax.twinx()
        sol["rad_219"].plot(ax=ax1, color="g", linewidth=0.75, drawstyle="steps-post")
        ...
        params.to_csv("full_model_4R4C_1meas.csv", index=True)
        sol.to_csv("simulation_traj_4R4C_1meas.csv", index=True)
        p_nom = pd.Series(
            data=param_est.p_nom,
            index=params.index
        )
        p_nom.to_csv("p_nom_4R4C_1meas.csv", index=True)
        print(params) 
