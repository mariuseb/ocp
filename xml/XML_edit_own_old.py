...
def read_variable(node):
  try:
    qn = qualified_name(node);
    return ca.variable(qn);
  except:
    raise NameError("...")

def qualified_name(nn):

  qn = ""
  for i, sub in enumerate(nn):
    # Add a dot
    if (i != 0):
      qn += "."

    # Get the name part
    qn += nn[i].attribute("name");
  ...
  def read_expr(node):
    ...
    if (name=="Add"): 
      ...
      return ca.acos(read_expr(node[0]))
    elif (name=="Asin"):
      return ca.asin(read_expr(node[0]))
    elif (name=="Atan"):
      return ca.atan(read_expr(node[0]))
    elif (name=="Cos"):
      return ca.cos(read_expr(node[0]))
    """
    elif (name=="Der"):
      return ca.variable(read_variable(node[0]).der_of).v;
    """
    elif (name=="Div"):
      return read_expr(node[0]) / read_expr(node[1])
    elif (name=="Exp")
      return ca.exp(read_expr(node[0]));
    elif (name=="Identifier")
      return read_variable(node).v
    """
    } elif (name=="IntegerLiteral" || name=="BooleanLiteral") {
      casadi_int val;
      node.get(&val);
      return val;
    } elif (name=="Instant") {
    ...
    """
      ...
      return read_expr(node[0]) <= read_expr(node[1]);
    elif (name=="LogGeq") { // Logical greater than equal
      return read_expr(node[0]) >= read_expr(node[1]);
    elif (name=="LogLt") { // Logical less than
      return read_expr(node[0]) < read_expr(node[1]);
    elif (name=="LogGt") { // Logical greater than
      return read_expr(node[0]) > read_expr(node[1]);
    elif (name=="Max") {
      return fmax(read_expr(node[0]), read_expr(node[1]));
    elif (name=="Min") {
      return fmin(read_expr(node[0]), read_expr(node[1]));
    elif (name=="Mul") { // Multiplication
      return read_expr(node[0]) * read_expr(node[1]);
    elif (name=="Neg") {
      ...
      return ca.pow(read_expr(node[0]), read_expr(node[1]));
    elif (name=="RealLiteral")
      double val
      node.get(&val)
      return val
    elif (name=="Sin")
      ...
      return var(t_.at(0))
    elif (name=="TimedVariable")
      return read_variable(node[0]).v
    
    elif (name=="FunctionCall")
      #// Get the name of the function
      fname = qualified_name(node["exp:Name"]);
      ca.casadi_warning("Function call to '" + fname + "' incomplete");
      #// Collect the arguments
      args = node["exp:Arguments"];
